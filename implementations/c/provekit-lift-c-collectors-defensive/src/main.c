...
static int lift_one_file(
    ...
    result = pk_c_walker_lift_source_with_options(path, source, options);
    free(source);
    if (!result) {
...
static void handle_initialize(const char *id) {
    send_response(id,
        ...
        "\"name\":\"c-collectors-defensive\",\"protocol_version\":\"pep/1.7.0\","
        "\"version\":\"0.1.0\"}");
}
...
static void handle_parse(const char *id, const char *line) {
    ...
    result = pk_c_walker_lift_source_with_options(
        path ? path : "source.c",
        source,
