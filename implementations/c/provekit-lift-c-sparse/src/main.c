...
static int lift_one_file(
    ...
    result = pk_c_sparse_lift_source_with_options(path, source, options);
    free(source);
    if (!result) {
...
static int walk_path(
    ...
    if (S_ISREG(st.st_mode)) {
        return has_suffix(path, ".c") ? lift_one_file(path, acc, options) : 0;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        rc = walk_path(child, acc, options);
        free(child);
        if (rc != 0) {
            closedir(dir);
            return rc;
...
static void handle_parse(const char *id, const char *line) {
    ...
    result = pk_c_sparse_lift_source_with_options(
        path ? path : "source.c",
        source,
...
static void handle_lift(const char *id, const char *line) {
    ...
    for (size_t i = 0; i < source_paths.len; i++) {
        ...
        rc = walk_path(resolved, &acc, &parse_config.options);
        free(resolved);
        if (rc != 0) {
