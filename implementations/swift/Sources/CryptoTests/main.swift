...
//   * CICP golden vectors from protocol/conformance/cicp
...
// MARK: - CICP conformance

struct CICPVectorCatalog: Decodable {
    let catalogVersion: String
    let catalogCid: String
    let `protocol`: String
    let vectors: [CICPVector]
}

struct CICPVector: Decodable {
    let name: String
...
func findCICPVectorDir() -> URL? {
    var dir = URL(fileURLWithPath: FileManager.default.currentDirectoryPath, isDirectory: true)
    while true {
        let candidate = dir
            .appendingPathComponent("protocol")
            .appendingPathComponent("conformance")
            .appendingPathComponent("cicp")
        if FileManager.default.fileExists(atPath: candidate.appendingPathComponent("vectors.json").path) {
            return candidate
        }
...
func readJSONValue(_ url: URL) throws -> Any {
    let data = try Data(contentsOf: url)
    return try JSONSerialization.jsonObject(with: data, options: [])
}

func jcsCanonical(from json: Any) throws -> JcsCanonical {
    if json is NSNull {
        return .null
    }
    ...
    if let array = json as? [Any] {
        return .array(try array.map { try jcsCanonical(from: $0) })
    }
    if let object = json as? [String: Any] {
        return .object(try object.map { key, value in
            (key, try jcsCanonical(from: value))
        })
    }
    throw NSError(
        domain: "CryptoTests.CICP",
        code: 1,
        userInfo: [NSLocalizedDescriptionKey: "unsupported JSON value \(type(of: json))"]
    )
}

func jsonObject(_ value: Any) -> [String: Any]? {
    return value as? [String: Any]
}

func stringField(_ body: [String: Any], _ field: String) -> String? {
    return body[field] as? String
}

func stringArrayField(_ body: [String: Any], _ field: String) -> [String] {
    guard let values = body[field] as? [Any] else { return [] }
    return values.compactMap { $0 as? String }
}

func appendCICPFields(_ body: [String: Any], scalarFields: [String], listFields: [String]) -> [String] {
    var out: [String] = []
    for field in scalarFields {
        if let cid = stringField(body, field) {
            out.append(cid)
        }
    }
    for field in listFields {
        out.append(contentsOf: stringArrayField(body, field))
    }
    return out
}

func requiredCICPInputCids(_ body: [String: Any]) -> [String] {
    switch stringField(body, "kind") {
    case "CIBlastRadius":
        return appendCICPFields(
            body,
            scalarFields: [
                "protocolCatalogCid",
                "jobDefinitionCid",
...
func validateCICPInputClosure(_ body: [String: Any]) -> String? {
    guard let rawInputs = body["inputCids"] as? [Any] else {
        return "inputCids must be an array"
    }
    var inputs = Set<String>()
    for (index, input) in rawInputs.enumerated() {
        guard let cid = input as? String else {
            return "inputCids[\(index)] must be a string"
        }
        inputs.insert(cid)
    }
    for cid in requiredCICPInputCids(body).sorted() {
...
test("cicp golden vectors derive pinned CIDs and fail closed") {
    guard let vectorDir = findCICPVectorDir() else {
        print("  could not locate protocol/conformance/cicp/vectors.json")
        return false
    }

    do {
        let manifestData = try Data(contentsOf: vectorDir.appendingPathComponent("vectors.json"))
        let manifest = try JSONDecoder().decode(CICPVectorCatalog.self, from: manifestData)
        if manifest.catalogVersion != "v1.6.2-2026-05-07" {
            print("  unexpected catalogVersion: \(manifest.catalogVersion)")
            return false
        }
        if manifest.catalogCid != "blake3-512:52bdb2be4b381cec2aff95db7755c84184878b45cd91882d262114a1abd2dd513f9ef3b250fb87093316fd0fcb48e4b97e109d463e57df5bda6aac0b1c719a0f" {
            print("  unexpected catalogCid: \(manifest.catalogCid)")
            return false
        }
        if manifest.protocol != "content-addressed-ci-protocol" {
            print("  unexpected protocol: \(manifest.protocol)")
            return false
        }

        for vector in manifest.vectors {
            if !vector.capability.hasPrefix("cicp.") {
                print("  \(vector.name): capability is not CICP namespaced")
                return false
            }

            let bodyURL = vectorDir.appendingPathComponent(vector.body)
            let json = try readJSONValue(bodyURL)
            guard let body = jsonObject(json) else {
                print("  \(vector.name): body must be a JSON object")
                return false
            }
            let closureError = validateCICPInputClosure(body)

            if !vector.shouldPass {
                guard let expected = vector.errorContains else {
                    print("  \(vector.name): missing errorContains")
                    return false
                }
                if closureError?.contains(expected) != true {
                    print("  \(vector.name): got error \(closureError ?? "nil"), want substring \(expected)")
                    return false
                }
                continue
            }
            ...
            if let closureError {
                print("  \(vector.name): \(closureError)")
                return false
            }
            guard let expectedCid = vector.expectedCid else {
                print("  \(vector.name): missing expectedCid")
                return false
            }
            let gotCid = computeJcsCid(try jcsCanonical(from: json))
            if gotCid != expectedCid {
                print("  \(vector.name):")
                print("    got:  \(gotCid)")
                print("    want: \(expectedCid)")
                return false
            }
        }
        return true
