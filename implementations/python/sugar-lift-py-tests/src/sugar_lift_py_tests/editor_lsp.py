...
#
# sugar editor LSP: a REAL editor language server for Python.
#
# This is the "red squiggle" gap, closed. The batch lift plugin (`lsp.py`) speaks
# the sugar-lift/1 NDJSON protocol the CLI spawns per invocation; it is NOT a
...
#
# The diagnostic IS the `sugar prove` verdict. On didOpen/didSave the server
...
# `== 5` from a vendor `.proof` in `.sugar/imports/`. `prove` reports that
...
# This server is a thin client: it spawns `sugar prove`, reads its machine
...
#
# This is DISTINCT from the rust `sugar-lsp` crate, which is the cross-language
# linker-daemon-routed editor server (roadmap). This one is the Python-kit-native
# server that renders `sugar prove` directly.
...
SERVER_NAME = "sugar-editor-lsp-python"
SERVER_VERSION = "0.1.0"
DIAGNOSTIC_SOURCE = "sugar"
...
def prove_diagnostics(text: str, path: str, report: dict) -> List[Dict[str, Any]]:
    """Turn a `sugar prove --json` report into diagnostics for THIS document.
    ...
    for row in report.get("rows", []):
        ...
        code = str(row.get("property") or "sugar.unsatisfied")
...
def find_project_root(path: str) -> Optional[str]:
    """Walk up from a file to the nearest directory containing `.sugar/`."""
    cur = os.path.dirname(os.path.abspath(path))
    while True:
        if os.path.isdir(os.path.join(cur, ".sugar")):
            return cur
...
def _sugar_binary() -> Optional[str]:
    env = os.environ.get("SUGAR_CLI")
...
def run_prove_report(path: str) -> Optional[dict]:
    ...
    THAT: the project's config + inherited `.sugar/imports/` proofs are copied
    ...
    editor shows no sugar diagnostics rather than a wrong one.
    """
    root = find_project_root(path)
    if root is None:
        return None
    binary = _sugar_binary()
    if binary is None:
        return None
    config = os.path.join(root, ".sugar", "config.toml")
    if not os.path.isfile(config):
        return None

    import tempfile

    workspace = tempfile.mkdtemp(prefix="sugar-lsp-")
    try:
        ws_sugar = os.path.join(workspace, ".sugar")
        ws_imports = os.path.join(ws_sugar, "imports")
        os.makedirs(ws_imports, exist_ok=True)
        shutil.copy(config, os.path.join(ws_sugar, "config.toml"))
        src_imports = os.path.join(root, ".sugar", "imports")
