...
This is the python sibling of `sugar-emit-java-junit` (PR-6) and the inverse
of the python harvester `sugar-lift-py-tests`: the harvester lifts pytest
...
`src/sugar_emit_python_pytest/predicate_table.py`, written in python. The
...
```
sugar-emit-python-pytest --rpc
```

Methods:

- `sugar.plugin.describe` — returns a full PEP 1.7.0 plugin memento
  `{envelope, header, metadata}` (NOT a flat object). The loader
  (`sugar-plugin-loader`) recomputes `header.cid` per §6.1 and refuses on
  ...
  the loader). This is what lets `sugar verify`/`materialize` load the kit
  through the standard plugin loader and invoke it to emit pytest gates.
- `sugar.plugin.invoke` — emit a pytest module from an emit plan in `params`.
...
```json
{
  ...
  "header": {
    "cid": "blake3-512:...",
    "content": {"name": "sugar-emit-python-pytest", "kind": "emit",
