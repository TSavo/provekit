...
The loader (``provekit-plugin-loader/src/loader.rs``) is language-agnostic: it
spawns this kit, sends ``provekit.plugin.describe``, and the ``result`` MUST be
...
floats) this is byte-identical to the rust ``provekit_canonicalizer::encode_jcs``
...
``provekit-plugin-loader/src/types.rs`` doc on ``PluginEnvelope``). Wire a real
...
def compute_plugin_cid(header: dict[str, Any]) -> str:
    ...
    Mirrors ``provekit-plugin-loader/src/cid.rs::compute_plugin_cid``.
    """
...
def plugin_memento() -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": {
            ...
            "source_url": "implementations/python/provekit-emit-python-pytest",
        },
    }
