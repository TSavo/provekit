"""Shared test fixtures for the python-sqlite3 realize kit.

As of the python-sqlite3 fan-out, `python-canonical-bodies-sqlite3.json` is
DELETED from the repo: the @ProveKitSugar shim `.proof` (carried over RPC via
...
covered WITHOUT depending on the deleted file.
"""
...
from provekit_realize_python_sqlite3 import realizer  # noqa: E402
from provekit_realize_python_sqlite3.realizer import BODY_TEMPLATE_REL  # noqa: E402
            ...
            "entries": [
                {
                    ...
                    "emission_template": {
                        "kind": "verbatim",
                        "template": (
                            "cursor = db.execute(${param0}, tuple(${param1}))\n"
...
def disk_fixture(
    ...
    """Stand up a temp repo root carrying a minimal canonical-bodies-sqlite3.json
