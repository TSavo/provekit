...
# sklearn.utils._testing assertion seat. Reuses the pytest seat's IR
# serialization so the emitted ir-document is byte-shaped identically; only the
# lift function differs (lift_file_sklearn_testing).
...
from .sklearn_testing_layer import lift_file_sklearn_testing

KIT_ID = "python-sklearn-testing"
KIT_VERSION = "0.1.0"
...
def handle_initialize(msg_id: Any) -> None:
    _send({
        ...
        "id": msg_id,
        "result": {
            "name": "provekit-lsp-sklearn-testing",
            "version": KIT_VERSION,
            "protocol_version": SHARED_LSP_PROTOCOL_VERSION,
            "kit_id": KIT_ID,
            "capabilities": {
                "source_surfaces": ["python-sklearn-testing"],
                "entry_kinds": [],
    ...
    try:
        ...
        for path in _iter_python_files(workspace_root, source_paths):
            try:
                ...
                continue
            out = lift_file_sklearn_testing(source, path)
