...
from provekit_lift_py_tests.canonicalizer import blake3_512_of, encode_jcs, vobj, vstr, vint, varr
from provekit_lift_py_tests.claim_envelope import (
    AuthoringKitAuthor,
    AuthoringLift,
    AuthoringLlm,
    ClaimEnvelope,
...
def _json_to_value(j):
    ...
    if isinstance(j, bool):
        from provekit_lift_py_tests.canonicalizer import vbool
        return vbool(j)
    if isinstance(j, int):
        return vint(j)
    if isinstance(j, str):
