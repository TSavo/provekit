...
from provekit_realize_python_aiosqlite.platform_semantics import (
...
# CursorLastRowid CID is identical to python-sqlite3 because kit_cid is elided from hash.
...
def test_aiosqlite_declaration_is_non_empty() -> None:
...
def test_aiosqlite_cursor_lastrowid_cid_matches_golden() -> None:
...
def test_aiosqlite_insert_tag_cid_matches_golden() -> None:
...
def test_aiosqlite_cid_matches_python_sqlite3() -> None:
    # Structural invariant: aiosqlite CursorLastRowid CID is identical to
    # python-sqlite3 CursorLastRowid CID because kit_cid is elided from hash.
    # This verifies the substrate kit_cid-elision rule.
    sqlite3_cursor_lastrowid_cid = (
        "blake3-512:"
        "6fbe68f4eb8a7cf5e58bd5859f43ce9bff042e3b68f85d6576fd3055d08f9d2a"
        "36bf9c316f6580111e178d82495b820b66414c934e5723d0e1c3a337df269933"
    )
    dvs = dimension_values()
    row_id = next(d for d in dvs if d["dimension_name"] == "RowIdMechanism")
    assert row_id["cid"] == sqlite3_cursor_lastrowid_cid, \
        "aiosqlite and python-sqlite3 CursorLastRowid must have identical CIDs (kit_cid elided)"


def test_aiosqlite_dimension_value_compare_to_structure() -> None:
...
def test_aiosqlite_rpc_dispatch_platform_semantics() -> None:
    # Positive: RPC dispatch returns correct shape for platform_semantics.
    response = dispatch({"jsonrpc": "2.0", "id": 5, "method": "provekit.plugin.platform_semantics"})
