...
//
// Binding-kit platform semantics declaration for the TypeScript pg library (node-postgres).
//
// Implements the provekit.plugin.platform_semantics RPC method (PEP 1.7.0).
// Returns the JSON payload for the "typescript-pg" binding target.
//
...
const KIT_ID = "provekit-binding-pg@0.1.0";
...
/** Returns the platform_semantics declaration for the pg binding kit. */
...
/** Returns the dimension values for the pg binding kit. */
function dimensionValues() {
  return [
    ...
    // return the specified columns (including the id) as a result row.
    _dimValue("RowIdMechanism", "ReturningClause", {
      kind: "atomic",
      name: "row_id_source",
      args: [
        {
          kind: "ctor",
          name: "returning_clause_result",
          args: [
            { kind: "ctor", name: "insert_result_set_at_call_return", args: [] },
          ],
        },
      ],
    }),
