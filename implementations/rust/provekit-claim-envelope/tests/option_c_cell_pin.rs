...
//
// CID-pinning tests for the concept:option<T> catalog scout.
//
// These tests pin the three BLAKE3-512 content addresses that constitute the
// M+N proof for the rust:Option<T> -> concept:option<T> -> c:tagged-union-macro
...
//   python3 menagerie/concept-shapes/scripts/mint_option.py
...
//
// The test reads the on-disk catalog envelope files produced by mint_option.py
...
const ABSTRACTION_CID: &str =
    "blake3-512:eae65d6f156fbd1dd194df1b37f2f059ceb40bd8e75dd94e582c2e8508d0f759\
     285721c8484cfba3c2435f451f8441ef83c6a6102203518ef478707b149a64a5";

const LIFT_CID: &str =
    "blake3-512:a829703f2ba7be255092e574b6bd040540336d7ceda380851bbb99dde2193c08\
     bb4cafbe650aba03e0c392c018e46687ae0e274862f39921904dc2064438d16d";

const REALIZE_CID: &str =
    "blake3-512:c550fb759564433d1ec92f9ca402dc4007c150b46e63385f9298eb836e6cc0a6\
     36f65dddaede1325852925bb72d8d570ae309e11281a3075f17749e9bc753807";
...
#[test]
fn pin_abstraction_concept_option() {
    let dir = catalog_root().join("abstractions");
    let filename = format!("concept:option.{ABSTRACTION_CID}.json");
    let path = dir.join(&filename);
    if !path.exists() {
        eprintln!(
            "SKIP pin_abstraction_concept_option: file not found (run `make mint` first):\n  {path:?}"
        );
        return;
    }
    ...
    assert_eq!(
        ...
        "concept:option abstraction CID drifted — re-run mint_option.py and update ABSTRACTION_CID"
    );
}

#[test]
fn pin_lift_rust_option_to_concept_option() {
    let dir = catalog_root().join("realizations");
    let filename = format!("rust:Option->concept:option.{LIFT_CID}.json");
    let path = dir.join(&filename);
    if !path.exists() {
        eprintln!(
            "SKIP pin_lift_rust_option_to_concept_option: file not found (run `make mint` first):\n  {path:?}"
        );
        return;
    }
    let actual = read_cid_from_envelope(&path);
    assert_eq!(
        actual, LIFT_CID,
        "rust:Option->concept:option lift CID drifted — re-run mint_option.py and update LIFT_CID"
    );
}

#[test]
fn pin_realize_concept_option_to_c() {
    let dir = catalog_root().join("realizations");
    let filename = format!("concept:option->c:tagged-union-macro.{REALIZE_CID}.json");
    let path = dir.join(&filename);
    if !path.exists() {
        eprintln!(
            "SKIP pin_realize_concept_option_to_c: file not found (run `make mint` first):\n  {path:?}"
        );
        return;
    }
    let actual = read_cid_from_envelope(&path);
    assert_eq!(
        actual, REALIZE_CID,
        "concept:option->c:tagged-union-macro CID drifted — re-run mint_option.py and update REALIZE_CID"
    );
}
