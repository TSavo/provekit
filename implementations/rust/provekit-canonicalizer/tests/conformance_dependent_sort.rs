...
fn to_cvalue(v: &serde_json::Value) -> Arc<CValue> {
    match v {
        ...
        serde_json::Value::Object(obj) => CValue::object(obj.iter().map(|(k, v)| (k.clone(), to_cvalue(v)))),
    }
}
...
fn rust_kit_produces_dependent_sort_pinned_cid() {
    ...
    let v: serde_json::Value = serde_json::from_str(&json_str).unwrap_or_else(|e| panic!("parse JSON: {}", e));
    let cv = to_cvalue(&v);
    let jcs = encode_jcs(&cv);
    let actual = blake3_512_of(jcs.as_bytes());
    let expected = fs::read_to_string(&cid_file).unwrap_or_else(|e| panic!("read cid file: {}", e)).trim().to_string();
