...
pub fn raw(s: &str) -> Expr {
    Expr::Raw(s.into())
}
pub fn lit(s: &str) -> Expr {
    Expr::LiteralString(s.into())
}
pub fn lit_str(s: &str) -> Expr {
    method(lit(s), "to_string", vec![])
}
pub fn ref_expr(expr: Expr) -> Expr {
...
pub fn let_typed(name: &str, ty: &str, expr: Expr) -> Stmt {
    Stmt::LetTyped {
        name: name.into(),
        ty: ty.into(),
        expr,
...
pub fn block_ret(expr: Expr) -> Block {
    Block {
        stmts: vec![ret(expr)],
    }
}
            ...
            body: block(vec![expr_stmt(Expr::Match {
                ...
                arms: vec![
                    crate::rust_gen::MatchArm {
                        ...
                        guard: None,
                        body: block(vec![expr_stmt(lit_str("Z"))]),
                    },
                    crate::rust_gen::MatchArm {
                        pattern: r#""String""#.into(),
                        guard: None,
                        body: block(vec![expr_stmt(lit_str("string"))]),
                    },
                    crate::rust_gen::MatchArm {
                        pattern: r#""Bool""#.into(),
                        guard: None,
                        body: block(vec![expr_stmt(lit_str("bool"))]),
                    },
                    crate::rust_gen::MatchArm {
                        pattern: "_".into(),
                        guard: None,
