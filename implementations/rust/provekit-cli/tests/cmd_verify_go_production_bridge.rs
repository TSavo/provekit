...
#[test]
fn go_production_path_refuses_planted_contradictory_implication() {
    if !go_available() {
        eprintln!("go not on PATH: skipping go contradictory-implication test");
        return;
    }
    if !z3_available() {
        eprintln!("z3 not on PATH: skipping go contradictory-implication test");
        return;
    }
    let lift_bin = build_go_lift_verify();
    let project = stage_go_project("contradiction", &lift_bin, 2);
    run_mint(&project);
