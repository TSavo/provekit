...
#[test]
fn ci_shadow_ignores_python_install_metadata_inside_source_roots() {
    let repo = make_shadow_repo("shadow-python-metadata");
    let before = run_ci_shadow(&repo, "rust");
    let before_cid = before["blastRadiusCid"].as_str().expect("blast cid");

    write_file(
        &repo.join("implementations/rust/provekit_lift_py_tests.egg-info/PKG-INFO"),
        "Name: provekit-lift-py-tests\nVersion: 0.0.0\n",
    );
    write_file(
        &repo.join("implementations/rust/__pycache__/generated.cpython-312.pyc"),
        "generated bytecode",
    );
    write_file(
        &repo.join("implementations/rust/.pytest_cache/v/cache/nodeids"),
        "[]\n",
    );

    let after = run_ci_shadow(&repo, "rust");
    let after_cid = after["blastRadiusCid"].as_str().expect("blast cid");
    assert_eq!(
        after_cid, before_cid,
        "CICP source closures must ignore Python installer/test metadata"
    );
