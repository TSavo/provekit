...
use clap::Parser;
use libsugar::core::emit_obligation::{build_bridge_body, member_envelope_canonical};
use libsugar::core::{
...
// Source-transform primitives live in libsugar (#1335 Phase A). The glob
...
pub(crate) use libsugar::core::source_transform::*;
...
impl SiteTransformKit for MaterializeKit<'_> {
    ...
    fn transform_site(&self, carrier: &CarrierComment) -> Result<SiteOutcome, String> {
        ...
        // libsugar) by re-serializing the carrier's raw payload. The
...
fn sync_materialize_bridge_proof(
    ...
    let proof_pool = sugar_verifier::load_all_proofs::run(project_root);
...
fn with_target_proof_cid(
    ...
    if sugar_verifier::types::memento_kind(target) != Some("contract") {
        return Err(format!(
...
// were moved to libsugar::core::emit_obligation as build_bridge_body
