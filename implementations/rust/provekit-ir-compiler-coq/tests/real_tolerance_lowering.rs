...
fn real_obligation_lowers_over_r_with_lra() {
    ...
    assert!(s.contains("Parameter a : R."), "`a` must be an R parameter:\n{s}");
    assert!(s.contains("Parameter b : R."), "`b` must be an R parameter:\n{s}");
    assert!(s.contains("Open Scope R."), "must open the R scope:\n{s}");
    assert!(s.contains("Require Import Reals") && s.contains("Lra"), "needs Reals + Lra:\n{s}");
    assert!(s.contains("lra."), "must discharge with lra, not lia:\n{s}");
    assert!(!s.contains("lia."), "lia is the integer tactic; must not appear:\n{s}");
}

#[test]
fn difference_is_infix_and_bound_is_an_exact_rational() {
    let s = compile(&tolerance_bound());
    assert!(s.contains("(a - b)"), "difference must be infix real subtraction:\n{s}");
    // 1.5 * 10^-7 = 15 / 10^8, exact, content-stable, no decimal literal.
    assert!(s.contains("(15 / 100000000)%R"), "positive bound as exact rational:\n{s}");
    assert!(s.contains("(- (15 / 100000000))%R"), "negative bound negates the rational:\n{s}");
    assert!(!s.contains("\"0.00000015\""), "the bound must not be a Coq string:\n{s}");
}
...
fn real_free_obligation_stays_on_z_and_lia() {
    ...
    let s = compile(&f);
    assert!(s.contains("Parameter x : Z."), "Real-free var stays Z:\n{s}");
    assert!(s.contains("lia.") && s.contains("Open Scope Z."), "integer path unchanged:\n{s}");
}
