...
// text string, array, map. Java peer of
// implementations/rust/provekit-proof-envelope/src/cbor.rs and
// implementations/csharp/Provekit.ProofEnvelope/Cbor.cs.
...
public final class Cbor {
    ...
    /** CBOR major types used by the substrate. */
    ...
    public static void appendHead(ByteArrayOutputStream out, int major, long arg) {
        if (arg < 0) {
            throw new IllegalArgumentException("CBOR head arg must be non-negative");
        }
        int mt = (major & 0x07) << 5;
        if (arg < 24) {
    ...
    public static void encodeTstr(ByteArrayOutputStream out, String utf8) {
        ...
        appendHead(out, MAJOR_TEXT_STRING, bytes.length);
        out.write(bytes, 0, bytes.length);
    }
