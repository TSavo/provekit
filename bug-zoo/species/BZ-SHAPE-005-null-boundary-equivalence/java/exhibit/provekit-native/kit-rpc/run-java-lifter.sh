...
contracts_jar="$java_root/provekit-java-contract-annotations/target/provekit-java-contract-annotations-0.1.0.jar"
...
mvn -f "$java_root/pom.xml" -pl provekit-java-contract-annotations,provekit-lift-java-core,provekit-lift-java-provekit-native -am package -DskipTests >&2

for jar in "$core_jar" "$adapter_jar" "$contracts_jar"; do
  if [[ ! -f "$jar" ]]; then
    echo "Missing required Java lifter jar: $jar" >&2
    echo "Try: mvn -f \"$java_root/pom.xml\" -pl provekit-java-contract-annotations,provekit-lift-java-core,provekit-lift-java-provekit-native -am package -DskipTests" >&2
    exit 1
  fi
done

java -cp "$core_jar:$adapter_jar:$contracts_jar" \
