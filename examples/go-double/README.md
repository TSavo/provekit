...
(`implementations/rust/sugar-cli/tests/cmd_verify_production_bridge.rs`). It
demonstrates that Sugar's verification spine is LANGUAGE-NEUTRAL: a Go
...
1. The **real Go lifter** (`sugar-lift-go-verify`, the verify-facing `go`
   lift surface registered in `.sugar/config.toml`) lifts:
...
2. `sugar mint` AUTO-WRITES the bridge `Double -> targetContractCid` for the
...
4. `sugar materialize` dispatches through `[[plugins]] kind = "realize"`
   plus `.sugar/realize/go/manifest.toml`. The Go realizer owns Go sugar
   assembly and Go module proof resolution; the CLI only sends normalized
   requests over RPC.
5. `sugar emit` dispatches through the same project registration. The
...
cd sugar-realize-go-core && go build -o /tmp/sugar-realize-go ./cmd/sugar-realize-go && cd ..
go build -o /tmp/sugar-emit-go-testing ./sugar-emit-go-testing/cmd/sugar-emit-go-testing
go build -o /tmp/sugar-emit-go-testify ./sugar-emit-go-testify/cmd/sugar-emit-go-testify
# point the .sugar manifests at these binaries, or put them on PATH, then
# from the repo root:
sugar mint   --project examples/go-double --out examples/go-double --no-attest
sugar verify --project examples/go-double --emit-witnesses /tmp/go-witnesses
sugar materialize --project examples/go-double --target go --library go \
  --source-dir /path/to/carriers --out-dir /tmp/go-materialized --compile-check
sugar emit --project examples/go-double --target go --framework testing \
  --plan /path/to/emit-plan.json --out-dir /tmp/go-emitted --compile-check
```

The gating integration test
(`implementations/rust/sugar-cli/tests/cmd_verify_go_production_bridge.rs`)
