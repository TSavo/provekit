...
//
// voltron-demo: smallest non-trivial two-vendor ProvekIt consumer.
//
//   M = 3 modules (ingest, persist, report) + a tiny bin entry.
//   N = 2 vendors (serde_json via provekit-shim-serde-json-rust,
//                  rusqlite   via provekit-shim-rusqlite).
//
// The spine `run_voltron_demo()` calls into each module; the modules
// own the boundary stubs that `provekit materialize` fills with vendor
...
// `provekit prove` runs, the pool unions:
//
//   1. voltron-demo.proof              (the head — this crate's spine)
//   2. provekit-shim-serde-json-rust.proof   (JSON lion)
//   3. provekit-shim-rusqlite.proof          (SQL  lion)
//
