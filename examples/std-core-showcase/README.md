...
- Spec: selected scalar direct call-result `assert_eq!` rows from std/core own
  ...
  - `coretests/tests/mem.rs`: generic type-arg-keyed rows for
    ...
    `size_of_basic` / `align_of_basic` vendor tests.
...
- Generic identity: type arguments are carried in the `#euf#` key for direct
  ...
  `align_of_basic` rows. Cfg-sensitive pointer-width variants remain out of
  scope until active-cfg evaluation lands.
...
| --- | ---: | --- |
| Generics | 30 | Closed for direct generic call-result identity in this slice by carrying type args in the `#euf#` key; cfg-sensitive pointer-width variants still count under CFG-sensitive until active-cfg evaluation lands. |
...
| Operator / expression RHS | 56 | `tests/atomic.rs::uint_and`: `assert_eq!(x.load(SeqCst), 0xf731 & 0x137f)`. |
