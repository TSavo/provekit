...
JIMFS_VERSION="${JIMFS_VERSION:-1.3.0}"
GUAVA_VERSION="${GUAVA_VERSION:-32.1.1-jre}"
JSR305_VERSION="${JSR305_VERSION:-3.0.2}"
CHECKER_QUAL_VERSION="${CHECKER_QUAL_VERSION:-3.37.0}"
JMH_VERSION="${JMH_VERSION:-1.37}"
MAVEN_BASE="${MAVEN_BASE:-https://repo1.maven.org/maven2}"

WORK_ROOT="$HERE/work"
PROJECT="$WORK_ROOT/commons-io-$LIB_VERSION"
PROOF_SCOPE="$WORK_ROOT/proof-scope"
JAR_DIR="${SUGAR_JAVA_REAL_LIB_JAR_DIR:-/tmp/sugar-java-real-lib-commons-io}"
LIB_SOURCES_JAR="$JAR_DIR/commons-io-$LIB_VERSION-sources.jar"
LIB_TEST_SOURCES_JAR="$JAR_DIR/commons-io-$LIB_VERSION-test-sources.jar"
...
JIMFS_JAR="$JAR_DIR/jimfs-$JIMFS_VERSION.jar"
GUAVA_JAR="$JAR_DIR/guava-$GUAVA_VERSION.jar"
JSR305_JAR="$JAR_DIR/jsr305-$JSR305_VERSION.jar"
CHECKER_QUAL_JAR="$JAR_DIR/checker-qual-$CHECKER_QUAL_VERSION.jar"
JMH_CORE_JAR="$JAR_DIR/jmh-core-$JMH_VERSION.jar"

if [ "${JAVA_REAL_LIB_COMMONS_IO_ON_REMOTE:-0}" != "1" ] \
  && [ "${JAVA_REAL_LIB_COMMONS_IO_USE_BCARGO:-1}" != "0" ] \
  && [ "$(uname -s)" != "Linux" ]; then
  echo "== run java real-library Commons IO on battleaxe via bcargo =="
  ...
  remote_cmd="cd $(printf '%q' "$remote_repo") && JAVA_REAL_LIB_COMMONS_IO_ON_REMOTE=1 JAVA_REAL_LIB_COMMONS_IO_SKIP_LOCAL_BUILD=1 examples/java-real-lib-commons-io/run.sh"
  ssh -o BatchMode=yes "$remote_host" "bash -lc $(printf '%q' "$remote_cmd")"
  exit $?
fi

if [ "${JAVA_REAL_LIB_COMMONS_IO_SKIP_LOCAL_BUILD:-0}" != "1" ]; then
...
echo "== fetch pinned Apache Commons IO source/test artifacts =="
fetch_jar "$LIB_SOURCES_JAR" "$MAVEN_BASE/commons-io/commons-io/$LIB_VERSION/commons-io-$LIB_VERSION-sources.jar"
fetch_jar "$LIB_TEST_SOURCES_JAR" "$MAVEN_BASE/commons-io/commons-io/$LIB_VERSION/commons-io-$LIB_VERSION-test-sources.jar"
...
fetch_jar "$JIMFS_JAR" "$MAVEN_BASE/com/google/jimfs/jimfs/$JIMFS_VERSION/jimfs-$JIMFS_VERSION.jar"
fetch_jar "$GUAVA_JAR" "$MAVEN_BASE/com/google/guava/guava/$GUAVA_VERSION/guava-$GUAVA_VERSION.jar"
fetch_jar "$JSR305_JAR" "$MAVEN_BASE/com/google/code/findbugs/jsr305/$JSR305_VERSION/jsr305-$JSR305_VERSION.jar"
fetch_jar "$CHECKER_QUAL_JAR" "$MAVEN_BASE/org/checkerframework/checker-qual/$CHECKER_QUAL_VERSION/checker-qual-$CHECKER_QUAL_VERSION.jar"
fetch_jar "$JMH_CORE_JAR" "$MAVEN_BASE/org/openjdk/jmh/jmh-core/$JMH_VERSION/jmh-core-$JMH_VERSION.jar"

echo "== prepare real Apache Commons IO $LIB_VERSION sources =="
rm -rf "$PROJECT" "$PROOF_SCOPE"
SRC_EXTRACT="$WORK_ROOT/commons-io-$LIB_VERSION-source-extract"
TEST_EXTRACT="$WORK_ROOT/commons-io-$LIB_VERSION-test-extract"
...
cp "$SRC_EXTRACT/META-INF/maven/commons-io/commons-io/pom.xml" "$PROJECT/pom.xml" 2>/dev/null || true
mkdir -p "$PROJECT/src/site/xdoc"
mkdir -p "$PROOF_SCOPE/src/test/java/org/apache/commons/io" "$PROOF_SCOPE/.sugar/vocab-exceptions"
cp "$PROJECT/src/test/java/org/apache/commons/io/ByteOrderMarkTest.java" \
  "$PROOF_SCOPE/src/test/java/org/apache/commons/io/ByteOrderMarkTest.java"
...
EXTRA_CP="$LANG3_JAR:$CODEC_JAR:$HAMCREST_JAR:$JUNIT_PIONEER_JAR:$BYTEBUDDY_JAR:$BYTEBUDDY_AGENT_JAR:$MOCKITO_INLINE_JAR:$MOCKITO_CORE_JAR:$OBJENESIS_JAR:$JIMFS_JAR:$GUAVA_JAR:$JSR305_JAR:$CHECKER_QUAL_JAR:$JMH_CORE_JAR:$PROJECT/src/test/resources"
...
echo "SCOPE: proving Apache Commons IO $LIB_VERSION with zero source changes."
echo "SCOPE: consistency axis covers exact assertion rows from a real Commons IO test-source subcorpus (ByteOrderMarkTest.java) learned from the real JUnit jar; unsupported/non-exact rows elsewhere are not claimed by this receipt."
echo "SCOPE: witness axis compiles and runs the real Commons IO JUnit suite; compiler facts and framework runtime behavior are not re-proven."
echo "real-lib=Apache Commons IO version=$LIB_VERSION java_files=$java_count test_files=$test_count jsr380_constraints=$jsr380_count"
if [ "$jsr380_count" = "0" ]; then
  echo "implication-edge stretch: skipped because this Commons IO artifact has no JSR-380 @Min/@Max/@Size/@NotNull method constraints."
fi
...
echo "== mint Apache Commons IO real source/test suite =="
...
if [ -z "$proof" ]; then
  echo "Apache Commons IO did not mint a proof" >&2
  exit 1
fi
...
echo "java real-library Commons IO self-check passed"
