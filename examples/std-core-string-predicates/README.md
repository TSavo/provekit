...
main `std-core-showcase`: point-wise string and ASCII-char predicate claims
from Rust's own tests or doctest source now lift as `#euf#` contract rows and
verify through z3 string theory.
...
- `library/coretests/tests/ascii.rs`: the string literal `str::is_ascii` rows
  from `test_is_ascii`.
...
- Iterator predicates such as `.chars().all(...)` and byte-slice predicates are
  outside this string-literal lane.
