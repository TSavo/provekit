...
A vendored boundary namespace under the ProvekIt proofchain. Every claim lives
in `provekit_shim_numpy/__init__.py`. Each binding carries **only sugar** — the
...
cd examples/provekit-shim-numpy
...
See `provekit_shim_numpy/__init__.py`. Keystone is `numpy.add`; the elementwise
