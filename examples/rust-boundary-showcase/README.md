...
vendor/   a path-dependency crate whose REAL source lives on disk. It is PLAIN
          rust — no `#[provekit::sugar]` tag, no provekit dependency. Its
          ...
          untouched `rot90`).
consumer/ a crate with a `#[provekit::boundary(library, call)]` stub
...
without a `#[provekit::sugar]` attribute is DERIVED into a
...
1. **derive-lift** the vendor with `PROVEKIT_LEAN_SOURCE=1` → a LEAN binding
...
`run.sh` self-checks BOTH verdicts and exits non-zero if provekit does not
...
```sh
cargo build -p provekit-cli --bin provekit -p provekit-walk --bin provekit-walk-rpc
./run.sh   # exits 0 on PASS
```

Everything is kit-side; the `.proof` is the transport; the rust substrate stays
proof-blind. The lift kit (`provekit-walk-rpc`) serves `lift` / `recognize` /
`provekit.plugin.materialize` as ONE source-oracle family — the same kit, the
