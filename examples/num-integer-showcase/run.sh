#!/usr/bin/env bash
# num-integer 0.1.46 showcase: real num-integer vendor rows lifted as Rust
# assertion consistency contracts and witnessed by re-running cargo test.
set -euo pipefail
...
echo "SCOPE: num-integer 0.1.46 exact vendor rows from doc-examples and Integer trait."
echo "SCOPE: GOOD claims are point-wise exact gcd/lcm/div_floor/mod_floor/is_multiple_of/is_even rows; BAD is a contradiction twin."
echo "SCOPE: residuals = extended_gcd rows, div_rem tuple rows, next_multiple_of rows, is_odd alias (mirrors is_even), lcm overflow behavior."
