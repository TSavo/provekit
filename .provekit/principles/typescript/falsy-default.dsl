...
//
// Tightened further (2026-04-27, #115 step 2.5): added is_in_dirty_set so the
// principle only fires on `||` expressions actually touched by the fix. One
// disagree on the manual-30 was express/Bug-21 where `req.baseUrl || ''` is
// stable code at the locus while the actual bug was a save/restore wrap
// elsewhere. Multi-require now supported by the grammar.

predicate any_param_decl($x: node) {
  match $b: node where binding.binding_kind == "param"
}

predicate any_falsy_default($x: node) {
...
principle falsy-default {
  ...
  require $dirty_witness: any_falsy_default($node)
    where is_in_dirty_set($node)
  report violation {
    at $node
    captures { node: $node }
