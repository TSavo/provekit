...
// JSON for protocolVersion `v1.2.0`, sign it under the v0 foundation
// seed, and write the signed attestation to
// `.provekit/catalog-signatures/v1.2.0.json`.
//
// Mirrors `sign_catalog.rs` but parameterized for v1.2.0. Same key,
// same declaredAt, additive bump (no breaking changes from v1.1.0).
...
use foundation_keygen::{
    ...
    signature_path_for, FOUNDATION_V0_SEED, V1_2_0_DECLARED_AT,
};

const PROTOCOL_VERSION: &str = "v1.2.0";
...
fn run() -> Result<(), String> {
    ...
    println!("# ProvekIt v1.2.0 catalog attestation");
    println!();
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("sign-catalog-v1-2-0: {e}");
            ExitCode::FAILURE
        }
    }
}
