...
// JSON for protocolVersion `v1.4.1`, sign it under the v0 foundation
// seed, and write the signed attestation to
// `.provekit/catalog-signatures/v1.4.1.json`.
//
// Mirrors `sign_catalog_v1_4_0.rs` but parameterized for v1.4.1. Same
// key, additive patch bump (re-sync only over v1.4.0; absorbs
// ir-formal-grammar spec-CID drift introduced by the normative Locus
// type addition; no protocol-level changes).
...
use foundation_keygen::{
    ...
    signature_path_for, FOUNDATION_V0_SEED, V1_4_1_DECLARED_AT,
};

const PROTOCOL_VERSION: &str = "v1.4.1";
...
fn run() -> Result<(), String> {
    ...
    println!("# ProvekIt v1.4.1 catalog attestation");
    println!();
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("sign-catalog-v1-4-1: {e}");
            ExitCode::FAILURE
        }
    }
}
