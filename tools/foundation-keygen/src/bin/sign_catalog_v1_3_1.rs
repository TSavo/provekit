...
// JSON for protocolVersion `v1.3.1`, sign it under the v0 foundation
// seed, and write the signed attestation to
// `.provekit/catalog-signatures/v1.3.1.json`.
//
// Mirrors `sign_catalog_v1_3_0.rs` but parameterized for v1.3.1. Same
// key, patch bump (re-sync only; no protocol-level changes from v1.3.0).
...
use foundation_keygen::{
    ...
    signature_path_for, FOUNDATION_V0_SEED, V1_3_1_DECLARED_AT,
};

const PROTOCOL_VERSION: &str = "v1.3.1";
...
fn run() -> Result<(), String> {
    ...
    println!("# ProvekIt v1.3.1 catalog attestation");
    println!();
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("sign-catalog-v1-3-1: {e}");
            ExitCode::FAILURE
        }
    }
}
