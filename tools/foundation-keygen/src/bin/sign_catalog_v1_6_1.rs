...
// JSON for protocolVersion `v1.6.1`, sign it under the v0 foundation
// seed, and write the signed attestation to
// `.provekit/catalog-signatures/v1.6.1.json`.
//
// Patch bump over v1.6.0: catalogs PEP as an extension-only protocol.
...
use foundation_keygen::{
    ...
    FOUNDATION_V0_SEED, V1_6_1_DECLARED_AT,
};

const PROTOCOL_VERSION: &str = "v1.6.1";
...
fn run() -> Result<(), String> {
    ...
    println!("# ProvekIt v1.6.1 catalog attestation");
    println!();
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("sign-catalog-v1-6-1: {e}");
            ExitCode::FAILURE
        }
    }
}
