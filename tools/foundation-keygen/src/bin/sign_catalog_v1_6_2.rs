...
// JSON for protocolVersion `v1.6.2`, sign it under the v0 foundation
// seed, and write the signed attestation to
// `.provekit/catalog-signatures/v1.6.2.json`.
//
// Patch bump over v1.6.1: catalogs CICP as an extension-only protocol.
// No core verifier behavior or cross-kit semantic obligation changes.
...
use foundation_keygen::{
    ...
    FOUNDATION_V0_SEED, V1_6_2_DECLARED_AT,
};

const PROTOCOL_VERSION: &str = "v1.6.2";
...
fn run() -> Result<(), String> {
    ...
    println!("# ProvekIt v1.6.2 catalog attestation");
    println!();
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("sign-catalog-v1-6-2: {e}");
            ExitCode::FAILURE
        }
    }
}
