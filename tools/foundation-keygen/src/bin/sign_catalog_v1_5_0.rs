...
// JSON for protocolVersion `v1.5.0`, sign it under the v0 foundation
// seed, and write the signed attestation to
// `.provekit/catalog-signatures/v1.5.0.json`.
//
// Mirrors `sign_catalog_v1_4_1.rs` but parameterized for v1.5.0. Same
// key, minor bump (sort grammar grow: FunctionSort + DependentSort
// variants added to the IR sort algebra; additive over v1.4.1, no
// breaking changes).
...
fn run() -> Result<(), String> {
    ...
    println!("# ProvekIt v1.5.0 catalog attestation");
    println!();
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("sign-catalog-v1-5-0: {e}");
            ExitCode::FAILURE
        }
    }
}
