...
- `exhibit/` is red: adding any supported contract surface exposes the missing edge to ProvekIt, and the verifier formula gate rejects the lab null witness against the lifted non-null requirement.
- `fixed/` is green: the same contract surface remains, the code is repaired, and the verifier formula gate discharges the paired non-null implication.
