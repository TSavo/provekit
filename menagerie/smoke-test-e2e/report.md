...
- **[CLOSED — Stub 1 — PR #715]** The driver's `formula_text_to_value` now calls `provekit_ir_symbolic::parse_expr` to lift each predicate string into a structured `IrFormula` AST before serializing. Operator mapping matches the kit's authoring API exactly (`<=` → atomic `"≤"`, `>=` → `"≥"`, `!=` → `"≠"`, `==` → `"="`, `<` → `"<"`, `>` → `">"`). Dotted identifiers (`result.is_ok`) are lifted as `Var`. Boolean literals in predicate position become nullary atomics. For strings that fail the parser (free-form prose), the function falls back to the prior single-atom encoding with a loud eprintln warning. The `SmokeTestResolver`'s wp_rules are also parsed from human-readable strings via `parse_expr` + `convert::formula_to_ir`, closing the formula-encoding gap on algebra-synthesis sites too. 29 inline round-trip tests confirm byte-identical JCS output.
- **[CLOSED — Stub 2]** The smoke-test driver now invokes the live `libprovekit::wp` evaluator for algebra-synthesis discharge. A `SmokeTestResolver` carries authored `wp_rule`s for `retry-loop` (`max_attempts >= 0 ∧ Q`) and `guard-then-commit` (`Q`). The evaluator reduces the rule against the postcondition placeholder and returns a ground formula; verdict is `exact`. The previously blocking condition (wp-as-formula PR series — PRs #619, #633, #663) is fully merged on main; this stub is now closed. Remaining loss on algebra-synthesis sites: the synthesized contract formula is still a single-atom shim (Stub 1 — now closed via `provekit-ir-symbolic::parse_expr`). The discharge ITSELF is exact; the formula encoding gap is now also closed.
