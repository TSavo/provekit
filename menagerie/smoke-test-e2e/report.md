...
- **[CLOSED — Stub 1]** The driver's `formula_text_to_value` now calls `provekit_ir_symbolic::parse_expr` to lift each predicate string into a structured `IrFormula` AST before serializing. Operator mapping matches the kit's authoring API exactly (`<=` → atomic `"≤"`, `>=` → `"≥"`, `!=` → `"≠"`, `==` → `"="`, `<` → `"<"`, `>` → `">"`). Dotted identifiers (`result.is_ok`) are lifted as `Var`. Boolean literals in predicate position become nullary atomics. For strings that fail the parser (free-form prose), the function falls back to the prior single-atom encoding with a loud eprintln warning. The `SmokeTestResolver`'s wp_rules are also parsed from human-readable strings via `parse_expr` + `convert::formula_to_ir`, closing the formula-encoding gap on algebra-synthesis sites too. 29 inline round-trip tests confirm byte-identical JCS output.
