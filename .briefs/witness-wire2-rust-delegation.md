# Next unit: `sugar prove` discharges a custom-witness contract via kit recompute

State at bank (2026-06-04, branch codex/python-proven-case-1-nullguard):
- Producer + recompute-discharge PROVEN at the kit level
  (`sugar-lift-py-pytest-witness`, commits 890dfd7e6 + fda48f1a5): emit a
...
1. **RPC method** `sugar.plugin.discharge_witness` (mirror lift/lift_implications):
...
5. **E2E acceptance through REAL `sugar prove`:** a project whose `.proof`
   carries a custom-witness contract → `sugar prove` discharges via kit
   recompute; mutate the impl → `sugar prove` REFUSES. Unit tests are not
   enough; the gate is the real command.

## Files to touch
- protocol/specs (RPC method shape), sugar-verifier/src (contract→obligation
  dispatch + verdict tier), sugar-lift-py-pytest-witness (mint surface + lsp
...
- Kit half is DONE + committed (fd6c3528a): producer, recompute, content-addressed
  ...
  `sugar-pytest-witness-discharge <witness.proof> <project> <code...>` ->
...
- Verdict enum: `sugar-verifier/src/types.rs:867` ObligationVerdict
...
- TWO real wiring decisions (the reason this is atomic, not a patch):
  1. project_dir: the consistency pass only gets the MementoPool, not the source
     tree. Thread the `sugar prove <project>` path down to verify_consistency
...
- Soundness backstop: the e2e discrimination gate (mutate impl -> `sugar prove`
  must REFUSE) empirically catches any branch error regardless of session state.

See [[project_sugar_witnessed_vs_consistency]], [[project_sugar_kit_owns_proof_resolution]], [[project_sugar_pin_all_three]].
