...
This paper's claim is empirical at three sites.

**Site one: the provekit codebase itself.** Paper 14's universal-correctness-bundle work landed `.proof` envelopes for the menagerie exhibits. Papers 18 and 19 landed concept-binding work and per-library realization patterns. The mementos described in §3 are not hypothetical; they are typed Rust structs in `provekit-ir-types`, parsed by per-family registry consumers in `libprovekit`, signed by the provenance keys catalogued in `reference_provekit_provenance_keys.md`. The corrected shape this paper names is the shape provekit has been operating in, made explicit.

**Site two: Stage 1 of N toward paper 22 §9.** The work in flight at the time of this paper's draft (codex `0b7877e8`, mint of SQL concept-shape catalog + TS realize kits for `better-sqlite3` and `pg`) is the cross-library cross-axis bootstrap for the envelope shape. When Stage 1 lands, the substrate has two TS realize kits keyed by `library_tag` (paper 22 PR #867's mechanism). The natural next packaging step is to extract the bootstrap kits as `@provekit/bindings-better-sqlite3` and `@provekit/bindings-pg` standalone npm packages, each with a `provekit.proof` envelope, each signed. That extraction is the empirical demo of §10's bootstrap shape.

**Site three: the four-consumer round-trip.** When Stage 3 lands (the call-graph effect-propagation engine that paper 22 §4 anchors), the verify, realize, migrate, and audit consumers will all read the same envelope. The receipt of a sqlite-to-pg migration will cite both libraries' `ConceptBindingMemento`s by CID, both libraries' `FunctionContractMemento`s by CID, and the `PromotionDecisionMemento` justifying each widening. The receipt's reproducibility is the empirical anchor of this paper's §6 claim.

Until those land, the paper's claim is the substrate's design articulated cleanly. The design is what the existing code already implements; what changes with this paper is the explicit naming and the negative list (no separate `.sugar` artifact).
