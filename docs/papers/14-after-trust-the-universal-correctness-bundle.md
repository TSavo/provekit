...
ProvekIt's existing C lifter chain already handles this class of source: `collectors-defensive`, `kunit`, `assertions`, `kernel-doc`, `sparse`, and `walk-c`. The point is not that each project is already polished into a perfect contract set. The point is stronger and more operational: there is no new class of engineering required to extend coverage to these projects. The work is ingestion. Clone the repository. Add a target stanza. Run the pipeline. Store the mementos.
