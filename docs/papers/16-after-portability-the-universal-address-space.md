...
The asm lifters make the same point one layer lower. [sugar-lift-asm-x86-64](../../implementations/rust/sugar-lift-asm-x86-64/README.md) lists `jne`, `jnz`, and related conditional branches in its core subset and states that `jne` lowers to the same control operation as C `if` and ARM `b.ne`. [sugar-lift-asm-aarch64](../../implementations/rust/sugar-lift-asm-aarch64/README.md) lists `cbz`, `cbnz`, and flag branches such as `b.ne`, and states that `cbz` and `b.ne` recover the same operation as C's `if`, modulo carrier and primitive operations.
