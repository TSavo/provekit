...
**Filed, architect-ruled, ready to dispatch:** #1068 (Path B of #1024). Architect ruling 2026-05-16: Option 2 confirmed (slow-test lane in CI; real Python/Java toolchain; no fixture stubs; no `#[ignore]`). Lane shape: separate `[[test]]` entry or `cfg(feature = "slow-tests")`, runner image must have python3 + javac + cc + Werror, fails loudly on missing toolchain (no silent skipping). Pre-decided architectural sentinel: BindKit invocation-determinism test must run BEFORE assertion 4 (separates "BindKit is deterministic" from "federation property holds"). Net Path B brief is locked. Ready to dispatch when Sir green-lights composition.
