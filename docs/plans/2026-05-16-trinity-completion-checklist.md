...
- **A9 #1075 federation type-erasure at hub (seam 4, architectural ruling needed).** `rust-lift` produces `NamedTerm { param_types: [i64, i64], return_type: i64 }`; `python-lift` produces `[Any, Any]` / `Any`. Bind CIDs diverge because NamedTerm types diverge. Architect recommendation: option (a), type erasure at the lifter boundary; types are emit-side reconstructions by the realize kit.
- **A10 #1076 Python source lifter operator-atom preservation (seam 4, substrate correctness bug).** Python source lift collides `x+y` and `x-y` bind CIDs because operator atoms are missing from bind-IR entries. **Worse than A9** because it's a lifter correctness violation, not a federation gap. Regression test asserts distinct operators produce distinct bind CIDs.
- **A11 #1077 Python realize body-template coverage (seam 3, mechanical).** Python realize plugin refuses 5 named concepts (`concept:conditional`, `concept:eq`, `concept:decl`, `concept:lt`, `concept:mul`) plus 2 UNNAMED-CONCEPT entries from `safe_divide_then_double`. Fix mints body templates for the 5 named concepts; surfaces the 2 unnamed entries (probably operator-fallback shapes the lifter produces, possibly tied to A10).
