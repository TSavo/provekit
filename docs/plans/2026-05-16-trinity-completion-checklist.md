...
**Composition test census staged on branch (NOT main; no PR yet):** `origin/1073-composition-test-census` at SHA 319ad456. 9 tests covering 4 seams (1, 3, 4, 6) against real subprocess toolchains in the slow-test lane (`cargo test ... --features provekit-cli/slow-tests`). CI workflow adds a `trinity-composition-census` job. Three `#[should_panic]` markers document the surfaced gaps; when those gaps close, the markers come off and the tests go fully green. Parallel-surfacing payoff: three integration gaps visible in one branch instead of discovered serially.

**Three gaps surfaced by the census, ready to file:**

- **A9 federation type-erasure at hub (seam 4, architectural ruling needed).** `rust-lift` produces `NamedTerm { param_types: [i64, i64], return_type: i64 }`; `python-lift` produces `[Any, Any]` / `Any`. Bind CIDs diverge because NamedTerm types diverge. Architect call pending on whether the hub erases types (option a, colimit-correct), preserves them (option b, federation bounded to within-type-system), or runs type-flow inference (option c, longer-arc post-Trinity work). Recommendation: option (a), type erasure at the lifter boundary, types are emit-side reconstructions by the realize kit.
- **A10 Python source lifter operator-atom preservation (seam 4, mechanical).** Python source lift collides `x+y` and `x-y` bind CIDs because operator atoms are missing from bind-IR entries. Fix preserves operator op-CIDs as bind-IR atoms; same fix-shape as A7. Regression test asserts distinct operators produce distinct bind CIDs.
- **A11 Python realize body-template coverage (seam 3, mechanical).** Python realize plugin refuses 5 named concepts (`concept:conditional`, `concept:eq`, `concept:decl`, `concept:lt`, `concept:mul`) plus 2 UNNAMED-CONCEPT entries from `safe_divide_then_double`. Fix mints body templates for the 5 named concepts; surfaces the 2 unnamed entries (probably operator-fallback shapes the lifter produces, possibly tied to A10).
