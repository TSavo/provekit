...
- **Per #1270 (substrate-uniform correction landed via PRs #1271-#1281):** kit declarations are NOT placed in libsugar Rust source files. Each kit owns its declaration; libsugar's job is to LOAD and COMPARE declarations via the `kit.platform_semantics` JSON-RPC method. New kits implement `kit.platform_semantics` in their plugin binary; libsugar's `platform_semantics_loader` fetches and caches the declaration at startup. The kit binary IS the declaration's source of truth.
...
- Substrate-side primitive (post-#1270): `implementations/rust/libsugar/src/core/platform_semantics.rs` (dispatcher + composition) + `implementations/rust/libsugar/src/core/platform_semantics_loader.rs` (JSON-RPC loader). The hardcoded per-kit Rust declaration files were deleted by #1271; declarations now ship in each kit binary's `kit.platform_semantics` RPC handler.
