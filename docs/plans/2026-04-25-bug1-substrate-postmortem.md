...
| v13 | D2 patch export                  | Oracles green, commit SHA logged, but `sugar-fix.patch` was 0 bytes. Target was a detached-HEAD worktree; targetRef fell back to literal `"HEAD"` string; baseRef captured that string; after commit advanced HEAD, `git diff HEAD..HEAD` resolved both ends to the same new commit. | 6531b06   |
...
| v16 | Overlay-bypass detector          | Agent hallucinated `/home/user/.sugar/...` for the first round of Writes, then ran `pwd`, saw the real overlay, and re-wrote everything to the correct path. Bypass detector caught the first Write and threw, even though the agent had already self-corrected. | 9d9f9d5   |
