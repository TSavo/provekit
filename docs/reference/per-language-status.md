...
| Surface | Status |
|---|---|
| Protocol catalog | `+` v1.6.6 catalog CID verified by `tools/recompute-spec-cids/` and `sugar verify-protocol --signed` |
...
|-------------|-----|------|---------------------------------------------------------|--------------------------|-------------------|----------------------|----------------------|
| Rust        | `+` | `+`  | `+ proptest, contracts, kani, prusti` ; `o creusot, flux`          | `-`      | `+`               | `+ sugar (canonical)` | `+`                  |
...
| C           | `+` | `~`  | `~`                                                     | `~`                      | `~`               | `~ (use Rust CLI)`   | `+`                  |
| Zig         | `+` | `~`  | `+ sugar-lift-zig (comment annotations)`             | `~`                      | `+`               | `~ (use Rust CLI)`   | `+`                  |
...
|-------------|---------------------------------------------|----------------------------|--------------------------|---------------------|--------------------------------------|----------------------------------|--------------------------|
| Rust        | `+ sugar-self-contracts`                 | `+`                        | `+ contract_cid`         | `+`                 | `+ canonical reference (mint_bridge_v14, BridgeDeclarationV14)` | `+ source-of-truth (PR #84)`    | `+`                      |
| Go          | `+ sugar-self-contracts`                 | `~ flat universal-claim-envelope` | `+ ContractCIDFromArgs` | `+ ComputeContractSetCID` | `~ flat targetContractCid; historical migration pending` | `~ Phase 2 in flight`           | `+`                      |
...
| Python      | `~ via sugar-lift-py-tests`              | `not assessed`             | `not assessed`           | `not assessed`       | `~ flat targetContractCid; historical migration pending` | `~ Phase 2 in flight`           | `-`                      |
| C++         | `+ sugar-self-contracts`                 | `~ flat universal-claim-envelope` | `+ contract_cid_from_args` | `+ compute_contract_set_cid` | `~ flat targetContractCid; historical migration pending` | `-`                              | `+`                      |
...
| Java / JVM  | `~ sugar-java-self-contracts (attestation pinned)` | `not assessed`       | `not assessed`           | `not assessed`       | `+ mintBridgeV14 (BridgeDeclarationV14)` | `-`                              | `-`                      |
| Ruby        | `~ mint-ruby-self-contracts (attestation pinned)` | `not assessed`           | `not assessed`           | `not assessed`       | `~ historical migration pending`           | `-`                              | `-`                      |
| C#          | `+ Sugar.SelfContracts`                  | `~ flat universal-claim-envelope` | `+ Mint.ContractCid`     | `+ contractSetCid in attestation` | `~ flat targetContractCid; historical migration pending` | `-`                              | `+`                      |
...
- **Bridge IR target**: tracks emission of the tagged-union `target` field per [`2026-05-03-bridge-target-dimensionality.md`](../../protocol/specs/2026-05-03-bridge-target-dimensionality.md). Rust now carries the canonical reference: `BridgeDeclarationV14` is defined in `protocol/sugar-ir.cddl`, the Rust IR types are regenerated from that grammar in `implementations/rust/sugar-ir-types/src/lib.rs`, and `sugar-claim-envelope::mint_bridge_v14` emits the layered envelope/header/body shape with the tagged-union target. The conformance fixture `bridge_decl_v1_4` in `conformance/fixtures.toml` pins the canonical bytes. The old flat `mint_bridge` and historical `bridge_decl` fixture are retained for back-compat per substrate-layers §4; new emissions use the layered tagged-union target. Other kits marked `historical migration pending` have not yet attached to that canonical bridge reference.

## Rust (canonical reference implementation)

**Kit:** `sugar-canonicalizer`, `sugar-claim-envelope`, `sugar-proof-envelope`, `sugar-ir-symbolic`. Shipping in the current v1.6.6 tree.

**Libs:** `sugar-verifier`, plus the kit crates above. Embedded verifier callable from any Rust crate via the public API.

**Lift adapters (shipping):**
- `sugar-lift-proptest`: walks `proptest!` blocks. Coverage: `prop_assume!`, `prop_assert!`, `prop_assert_eq!`, `prop_assert_ne!`.
- `sugar-lift-contracts`: walks `#[requires(...)]`, `#[ensures(...)]`, `#[invariant(...)]` macros from the `contracts` crate.

**Lift adapters (shipping):**
- `sugar-lift-kani`: walks `#[kani::proof]` functions, `kani::assume`, `kani::assert`.
- `sugar-lift-prusti`: walks `#[prusti_contracts::requires/ensures]`.

**Lift adapters (under evaluation):**
- `sugar-lift-creusot`: rich logical fragments; some annotations route through Tier 3 of the handshake.
- `sugar-lift-flux`: refinement types; partial lift.



**Embedded verifier:** Yes. `sugar_verifier::run(project_root)` returns a `HandshakeReport` synchronously.

**CLI:** `sugar` is the canonical Rust CLI; run `sugar verify-protocol` to confirm the protocol catalog CID it conforms to. Run `sugar --help` for the authoritative subcommand list; it includes `prove`, `verify`, `verify-protocol`, `protocol`, `package`, `lift`, `mint`, `dump`, `hash`, `implicate` (alias `imp`), `compose`, `recognize`, `materialize`, `bind`, `emit`, `self-check`, `doctor`, `release-gate`, `init`, and `version`. `lift` dispatches the configured lift-plugin protocol and writes ProofIR term JSON; `mint` is the separate composition step that envelopes lifted terms into a signed `.proof`. Bug Zoo is repo-owned machinery under `menagerie/bug-zoo/`, not a public `sugar` subcommand. Distributed from source today with `cargo install --path implementations/rust/sugar-cli`; crates.io publishing remains future work.
...
**Lift adapters (shipping):**
- `sugar-lift-zod`: walks `z.object`, `z.string`, `z.number`, validator combinators. Full chain decoder for all major zod methods.
- `sugar-lift-class-validator`: walks decorator-annotated class fields (`@IsNotEmpty`, `@MinLength`, `@Min`, `@Max`, `@IsEmail`, etc.).
- `sugar-lift-fast-check`: walks `fc.assert(fc.property(...))` blocks. Lifts property tests to `forall` contracts.
...
**Decorator macros (planned):** A `@sugar.contract(...)` decorator for direct authoring when no lift target exists.
...
**CLI:** Native TypeScript CLI planned. Use the Rust CLI (`sugar prove`) for verification; the TypeScript libs handle authoring and lifting.

**LSP Plugin:** Yes. `sugar-lsp` implements the Sugar NDJSON LSP plugin protocol (daemon mode over stdio) with `initialize`, `parse`, and `shutdown`.

## Go

**Kit:** Shipping in the current v1.6.6 tree. `implementations/go/sugar-ir-symbolic` provides the IR library. The canonicalizer matches the Rust implementation byte-for-byte.
...
**Lift adapters (shipping):**
- `sugar-lift-validator`: walks `validate:` struct tags from `go-playground/validator`.

**Lift adapters (planned):**
- `sugar-lift-ozzo`: walks `ozzo-validation` rule chains.

**Decorator macros:** Go has no decorator syntax. Comment-block annotations (`//sugar:contract`) under evaluation.

**Embedded verifier:** Yes. CGO bridge to the Rust canonicalizer; pure-Go canonicalizer planned.

**CLI:** Deferred. Use the Rust CLI.

**LSP Plugin:** Yes. `sugar-lsp-go` implements the Sugar NDJSON LSP plugin protocol with `initialize`, `parse`, and `shutdown`. Scans Go source for `//sugar:` annotations.

## C++

**Kit:** Shipping in the current v1.6.6 tree. `implementations/cpp/sugar-ir-symbolic` plus the canonicalizer. Header-only IR library; CMake integration shipped.

**Libs:** Shipping. Embedded verifier links into existing C++ projects.

**Lift adapters (shipping):**
- `sugar-lift-cpp-contracts`: walks `[[expects:]]` and `[[ensures:]]` attributes (C++26 contract syntax).

**Lift adapters (under evaluation):**
- `sugar-lift-assert-h`: walks `assert(...)` macros; coverage is partial because `assert.h` discards conditional information at compile time.
...
**LSP Plugin:** Yes. `sugar-lsp-cpp` implements the Sugar NDJSON LSP plugin protocol with `initialize`, `parse`, and `shutdown`.

## Python

**Kit:** Shipping in the current v1.6.6 tree. `implementations/python/sugar-lift-py-tests` provides the IR library, canonicalizer (JCS + BLAKE3-512), Layer 2 lift adapter, decorator macros, Pydantic lift adapter, and embedded verifier.
...
**Lift adapters (shipping):**
- `sugar.lift.pydantic`: walks `BaseModel` field annotations and `Field` constraints. Emits the same IR as Bean Validation `@Min`/`@Max`/`@Pattern`/`@Size` for equivalent constraints.
...
**Lift adapters (planned):**
- `sugar-lift-deal`: walks `@deal.pre`, `@deal.post`, `@deal.raises`.
- `sugar-lift-hypothesis`: walks `@given(...)` test functions; shape similar to proptest.
- `icontract`, `attrs`, `dataclasses-json` schemas.

**Decorator macros:** `@sugar.contract(pre=..., post=..., inv=...)` ships for direct authoring. Supports both string expressions and callable predicates (with runtime checking).

**Embedded verifier:** Yes. Delegates to the Rust `sugar` CLI via subprocess, ensuring full protocol conformance without reimplementing the verifier.

**CLI:** Deferred. Use the Rust CLI.

**LSP Plugin:** Yes. `sugar.lsp` implements the Sugar LSP plugin protocol (NDJSON over stdio) with `initialize`, `parse`, and `shutdown` methods.

## Java / JVM

**Kit:** Shipping in the current v1.6.6 tree. Multi-module Maven project with SLF4J-style architecture: `sugar-lift-java-core` (facade) + per-annotation binding JARs, discovered via `java.util.ServiceLoader`.

**Libs:** Shipping. `sugar-ir` provides IR types (`Formula`, `Term`, `Sort`, `Declaration`, `IrDocument`, `CallEdgeDecl`, plus the layered `BridgeDeclarationV14` / `BridgeHeaderV14` / `BridgeMetadataV14` / `BridgeEnvelope` / `BridgeTarget` records).

**Bridge IR compatibility:** `sugar-claim-envelope::ClaimEnvelope.mintBridgeV14` emits the layered envelope/header/body shape with the tagged-union `target` field. Byte-identical to the Rust canonical reference; pinned against the `bridge_decl_v1_4` conformance fixture in `BridgeV14RoundtripTest`. Legacy bridge shapes remain available only for back-compat with historical bytes. Closes #188.

**Lift adapters (shipping):**
- `sugar-lift-java-bean-validation`: walks `@NotNull`, `@Email`, `@Min`, `@Max`, `@Pattern`, `@Size`, `@Positive`, `@Negative`, `@AssertTrue`, `@AssertFalse`, `@DecimalMin`, `@DecimalMax`, `@Digits`, `@Future`, `@Past`.
- `sugar-lift-java-junit`: walks JUnit Jupiter test assertions and emits point-specific value-scope implications. `assertEquals(42, parseInt("42"))` lifts only the witnessed term; local assignments become SSA binding facts and branch joins become guarded implications with opaque branch-condition atoms.
- `sugar-lift-java-jml`: walks `//@ requires`, `//@ ensures`, `//@ invariant` comment-block annotations. Uses a hand-written tokenizer + recursive-descent parser (no regex gymnastics) to produce structured IR that is byte-for-byte identical to Bean Validation for equivalent constraints.
- `sugar-lift-java-spring-web`: walks `@RequestParam`, `@PathVariable`, `@RequestMapping`, etc.
- `sugar-lift-java-cofoja`: walks `@Requires`, `@Ensures`, `@Invariant` annotations.
...
**Kit:** Shipping. `implementations/c/sugar-ir` provides the IR library, JCS canonical JSON emitter, and BLAKE3-512 hash wrapper.
...
**LSP Plugin:** Yes. `sugar-lsp-c` implements the Sugar NDJSON LSP plugin protocol (sugar-lsp-plugin/1 over stdio) with `initialize`, `parse`, and `shutdown`.

## Zig

**Kit:** Shipping in the current v1.6.6 tree. `implementations/zig/sugar-ir` provides the IR library with JCS canonical JSON serialization and BLAKE3-512 hashing via `std.crypto.blake3`.

**Libs:** Under evaluation.

**Lift adapters:** `sugar-lift-zig` scans `//sugar:contract`, `//sugar:implement`, and `//sugar:verify` annotations in Zig source files. Emits JCS canonical IR.
...
**LSP Plugin:** Yes. `sugar-lift-zig --rpc` implements the Sugar NDJSON LSP plugin protocol with `initialize`, `parse`, and `shutdown`.

## Ruby

**Kit:** Shipping in the current v1.6.6 tree. `implementations/ruby/lib/sugar/ir.rb` provides IR types, JCS canonical JSON emitter, and BLAKE3-512 hashing. Requires Ruby 3+ (uses endless-method syntax); macOS system Ruby 2.6 cannot parse the kit. Conformance harness prefers Homebrew Ruby automatically.

**Libs:** Under evaluation.

**Lift adapters (shipping):**
- `sugar/lift/active_model`: walks `validates :field, presence: true, length: { minimum: N }` declarations.
- `sugar/lift/dry_validation`: walks `Dry::Validation::Contract` rule definitions.
- `sugar/lift/rspec`: walks `RSpec.describe` blocks; lifts `it { is_expected.to ... }` matchers.
...
**LSP Plugin:** Yes. `bin/sugar-lsp-ruby` implements the Sugar NDJSON LSP plugin protocol.
...
**Kit:** Shipping in the current v1.6.6 tree. `implementations/csharp/Sugar.IR`, `Sugar.Canonicalizer`, `Sugar.SelfContracts`, `Sugar.ClaimEnvelope`, `Sugar.ProofEnvelope`, `Sugar.Verifier`. Multi-project .NET 10 solution with full IR + canonicalizer parity to Rust.

**Libs:** Shipping. `Sugar.Verifier` is the in-process verifier.

**Lift adapters (shipping):**
- `Sugar.Lift.DataAnnotations`: walks `[Required]`, `[StringLength]`, `[Range]`, `[RegularExpression]`, `[EmailAddress]`, etc.
- `Sugar.Lift.Linq`: walks LINQ expression trees and lifts predicate quantifiers (`All`, `Any`) to `forall`/`exists` IR.

**Decorator macros:** .NET attributes are the natural authoring surface. Lift adapters consume them directly.

**Embedded verifier:** Yes. `Sugar.Verifier` (in-process verifier) ships as part of the .NET 10 solution.

**CLI:** Deferred. Use the Rust CLI.

**LSP Plugin:** Yes. `Sugar.Lsp.Plugin` implements the Sugar NDJSON LSP plugin protocol.

**Bridge IR gap:** `Sugar.IR.Collector.BridgeDecl` is `(TargetContractName, IrArgSorts, IrReturnSort)`, a lift-adapter helper, not the spec Bridge shape. The migration target is the layered shape with a tagged-union `target` field. Self-contracts attestation IS signed (the bundle CID is pinned, and `contractSetCid` is emitted), but Phase 2 cross-kit bridges require a separate spec-shaped `BridgeDeclaration` record to be added.

## Swift

**Kit:** Shipping in the current v1.6.6 tree. `implementations/swift/Sources/Sugar/IR.swift` provides IR types, JCS canonical JSON via `Jcs.encode`, and BLAKE3-512 hashing. The conformance runner at `Sources/ConformanceRunner/main.swift` validates byte-identical emission against the canonical Rust output for `eq_atomic`, `pattern1_bounded_loop`, `contract_decl`, `bridge_decl`.
...
**Kit:** In progress (`feat/php-kit`). `implementations/php/sugar-ir-symbolic` provides IR types, JCS canonical JSON emitter, and BLAKE3-512 hashing. `implementations/php/sugar-lift` provides a lift adapter and LSP daemon. Self-contracts orchestrator structure is in place (`implementations/php/sugar-self-contracts`); no pinned attestation yet.

**Libs:** Under evaluation.

**Lift adapters:** In progress. `implementations/php/sugar-lift/src/lifter.php` is the active lift adapter; coverage of PHP annotation libraries under evaluation.
...
**LSP Plugin:** In progress. `implementations/php/sugar-lift/src/lspd.php` implements the Sugar NDJSON LSP plugin protocol; not yet shipped.
...
This document lives in the repository's `docs/` directory and is updated per release. The authoritative protocol catalog is at `protocol/specs/2026-04-30-protocol-catalog.json`; verify the local install conforms via `sugar verify-protocol`.
