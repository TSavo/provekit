...
- **Contract lifter** — reads the vendor's own tests/source and lifts the *claims* into contract
  ...
  (a spec exists). [`sugar-lift-rust-tests`, `sugar-lift-python-source`, `sugar-lift-java-tests`
  (Java-native on `com.sun.source`: vocab learned from framework source via throw-locus, loops→∀,
  numeric/string universes generalized from vendor bodies; rebuilt after the prior Rust crate that
  hand-scanned Java text — a bespoke Java grammar — was torn down)]
...
- **Bridge lifter** — lifts a cross-language / FFI call edge into a `CallEdgeDecl`
  ...
  [`cpython_ctypes_resolver`, the go cgo resolver, the Java-native `JavaPanamaFfmRpc` (reads the
  Panama downcall from the `com.sun.source` AST — `downcallHandle`→`find("symbol")`→`invokeExact`,
  no regex — and emits the `call-edge`; a Java claim that contradicts the bound Rust contract
  refutes across the bridge)]
- **Witness lifter (kit oracle)** — WITNESSES THE PROGRAM: it resolves the witness body — the
  ...
  Java-native `JavaJunitWitnessRpc` (resolve_witness + recompute; the lying-discharge twin is
  refused on the blake3 mismatch)]
