...
There is no `sugar-lsp-java` directory, binary, or module anywhere in `implementations/java/`. No file matching `*lsp*`, `*Lsp*`, or `*forward*` exists in the Java kit tree.
...
### Existing RPC infrastructure: `sugar-java-self-contracts/src/main/java/com/sugar/selfcontracts/Rpc.java`
...
```java
// implementations/java/sugar-java-self-contracts/src/main/java/com/sugar/selfcontracts/Rpc.java:44-60
...
### Self-contracts module: `sugar-java-self-contracts/`

Standard Maven layout under `implementations/java/sugar-java-self-contracts/`:

```
sugar-java-self-contracts/
  src/main/java/com/sugar/selfcontracts/
...
2. Walk the AST for `// @sugar-contract` annotations or Javadoc tags
3. Extract method signatures, parameter types, return types
4. Lift to Sugar IR (`Declaration`, `ContractDecl`)

The `sugar-ir` module provides the Java IR types:

```
sugar-ir/src/main/java/com/sugar/ir/
  Sort.java
  IrDocument.java
  Declaration.java
...
| Module | Purpose |
|--------|---------|
| `sugar-ir` | IR type definitions (Sort, Declaration, BridgeV14) |
| `sugar-claim-envelope` | Claim envelope types |
| `sugar-java-self-contracts` | Self-contracts minting + RPC handler |
| `sugar-lift-java-core` | Core lift infrastructure |
| `sugar-lift-java-bean-validation` | Bean validation lift |
| `sugar-lift-java-jml` | JML annotation lift |
| `sugar-lift-java-cofoja` | Cofoja contract lift |
| `sugar-lift-java-spring-web` | Spring Web lift |
| `sugar-lift-java-spring-security` | Spring Security lift |
| `sugar-lift-java-swagger` | Swagger/OpenAPI lift |
| `sugar-lift-java-jackson` | Jackson annotation lift |
| `sugar-lift-java-jpa` | JPA annotation lift |
| `sugar-lift-java-hibernate` | Hibernate annotation lift |
...
- Package naming: `com.sugar.<module-name>` (e.g., `com.sugar.selfcontracts`, `com.sugar.ir`).
- No `sugar-lsp-java` module exists yet; all lift modules follow the `sugar-lift-java-*` naming convention.
...
1. **Should the LSP plugin be a new Maven module `sugar-lsp-java/` or added to an existing module?**

   Proposed: new module `sugar-lsp-java/` following the existing naming convention. This keeps the LSP surface separate from lift modules and mirrors the Go kit's `cmd/sugar-lsp-go/` pattern. The module would depend on `sugar-ir` and `javaparser-symbol-solver-core`.

2. **What should the binary entry point be?**

   Proposed: `sugar-lsp-java/src/main/java/com/sugar/lsp/Main.java` with a `main()` method that runs the NDJSON loop. The Maven `appassembler-maven-plugin` or `maven-shade-plugin` can produce an executable JAR. The ForwardPropagator will spawn it as `java -jar sugar-lsp-java-0.1.0.jar`.
...
3. **Protocol shape mismatch (#298):** The lift protocol uses `initialize` + `lift` while LSP uses `initialize` + `parse`. Should the Java LSP support both in one binary, or should they be separate?
   ...
   Proposed: separate binaries. The lift protocol binary already exists (`Rpc.java` in self-contracts). The LSP binary should only support `initialize`, `parse`, and `shutdown`. This matches the Go kit's separation (`cmd/sugar-lsp-go/` vs `cmd/mint-go-self-contracts/`).

4. **Does the Java LSP need to support annotation scanning beyond `// @sugar-contract`?**
