...
- Updated `provekit-walk` term emission so qualified Rust paths lower as `kind: "fully-qualified-path"` with `concept: "concept:fully-qualified-path"` instead of truncating to the leaf name.
...
- `cargo test --manifest-path implementations/rust/Cargo.toml -p provekit-walk --test term_emit_d2 -- --nocapture`
- `cargo test --manifest-path implementations/rust/Cargo.toml -p provekit-walk --test term_emit_d3 -- --nocapture`
...
- `cargo test --manifest-path implementations/rust/Cargo.toml -p provekit-walk --tests`
- `cargo test --manifest-path implementations/rust/Cargo.toml -p provekit-walk charon_runner::tests::invokes_charon_on_inline_source_and_finds_function_f -- --nocapture`
- `git diff --check`
...
`gh issue view 962 -R TSavo/provekit --json body --jq '.body'` failed because the sandbox cannot connect to `api.github.com`. Work proceeded from the issue body supplied in the task brief.

`git add` failed with:

```text
fatal: Unable to create '/Users/tsavo/provekit/.git/worktrees/pk-962-trait-path/index.lock': Operation not permitted
```
