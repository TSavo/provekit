...
| D5h | **`concept:insert-and-get-id` is missing from ALL THREE target body-template catalogs.** Verified: ts-pg, python-sqlite3, python-aiosqlite all have only `concept:sql-query` and `concept:sql-execute`. The `recordEvent` callsite (classified as `concept:insert-and-get-id`) cannot find a matching template on any target, so 3/12 cells stay MISSING regardless of plugin fixes. | Mint `concept:insert-and-get-id` entries in each catalog parameterized by `(sql, args)` with `min_params=2 / max_params=2`. Each plugin's body template owns its dialect's emission: pg's renderer substitutes `?` for `$N` and appends ` RETURNING id`, then wraps with `pool.query(...)`. sqlite3 and aiosqlite take the SQL as-is and wrap with `cursor.execute(...); return cursor.lastrowid`. Both differences are EMISSION-time text-level concerns inside per-plugin templates; no SQL parser, no dialect-translation primitive, no structured-SQL substrate evolution. The substrate carries SQL strings opaquely through `${param0}`; each plugin's renderer handles its own dialect's needs. (Per-plugin dialect emission ratified 2026-05-19 over the four demo callsites; rejected the SQL-as-structured-content path as out of scope for D5h after grepping the corpus.) | Flips 3 recordEvent cells from MISSING to SEMANTIC or MATCHES. Closes the catalog gap empirically; no new substrate primitives needed. | #1254 |
...
| D13a | Ship Trinity-demo library kits as shim packages (phase B): `java-sqlite-jdbc-proof.jar`, `provekit-shim-python-sqlite3` (pip), `provekit-shim-rusqlite` (cargo). Ruling: `docs/plans/2026-05-19-trinity-shim-distribution-ruling.md` (RULED 2026-05-19). Four ecosystem-specific architect calls resolved via existing substrate machinery (rank-N pinning + admission tiers + kit-dispatch tiering). Sub-issues D13a-Cargo / D13a-Pip / D13a-Maven / D13a-Npm to file. | `binding_semantics_for_tag` resolves library tags via shim package discovery + .proof read. No hardcoded arms. New `dispatch_shim_resolve(library_tag, ecosystem)` primitive call slots into existing `kit_dispatch.rs:521-583` at tier 3.5. | Substrate consumes library kits via vendor-ecosystem package managers without requiring vendor cooperation. | #1237 (meta); ruling: `2026-05-19-trinity-shim-distribution-ruling.md` |
...
| D16 | Value-tier op family mints for non-classical architectures (quantum first: `concept:superposition`, `concept:basis-state`, `concept:complex-amplitude`, `concept:measure`, sort mints `Qubit` and `QubitRegister<N>`; cellular automata, dataflow, neuromorphic, stack-machine, combinator architectures follow). Per `docs/explanation/value-tier-op-family.md`. | Each new architecture mints its own value-tier ops via the existing concept-catalog + per-kit declaration machinery; no substrate primitive changes. Cross-family migration (classical ↔ quantum etc.) uses the same trichotomy. | Substrate's open-extensibility claim becomes load-bearing with a second-architecture proof point. | not yet filed (forward roadmap) |
| D17 | Domain-specific propagation primitives for genuinely non-call-graph architectures (dataflow, cellular, neuromorphic, combinator) per rules doc §10.1. When a problem-domain crosses the call-graph boundary, file a ruling defining the new propagation primitive's contract; the substrate's dispatcher routes per (source-arch, target-arch) pair. Planned protocol capability, not an exception. | Adds new propagation primitives alongside `propagate_effects`; dispatcher gains routing logic to select the right primitive per architecture pair. | Substrate's protocol fully admits the multi-domain propagation model. | not yet filed (forward roadmap; depends on D16 surfacing a non-call-graph kit) |
| D18 | Structural-tier op family mints (`concept:import`, `concept:type-declaration`, `concept:type-alias`, `concept:interface-declaration`, `concept:function-signature`, `concept:helper-function-body`, etc.). Generalizes the value-tier op family (D16) pattern to non-callsite source constructs. Each structural concept mints in `menagerie/concept-shapes/catalog/algorithms/`; per-language realize plugins emit per-target syntax. Dissolves `cmd_bind_migrate::compose_migrated_source`'s hardcoded structural shell. | Migrate's structural shell becomes substrate-derived (every non-callsite construct lifts to concept:* and realizes per target). The Trinity demo continues working on hardcoded shell during interim; this row delivers the substrate-uniform endgame. | Cross-language migration of WHOLE FILES (not just callsites) becomes substrate-uniform. The migrate workflow shrinks to pure composition; no corpus-specific scaffolding. | not yet filed (forward roadmap) |
| D19 | Project-scaffolding-via-substrate-concepts (`concept:package-manifest`, `concept:build-config`, `concept:test-runner-config`, `concept:lockfile`). Cross-binding-pair scaffolding (e.g., package.json → Cargo.toml) becomes substrate-derivable instead of hardcoded per-(source, target) in workflow code. Sibling to D18 (structural-tier op family); specifically targets package-manager / build-system artifacts. | Migrate's `write_typescript_project` / `write_python_project` style scaffolding generation becomes substrate-derived. Project-conversion workflows compose substrate concepts without per-pair hardcoding. | Complete dissolution of per-(source, target) scaffolding code in migrate workflows. | not yet filed (forward roadmap; depends on D18 establishing the structural-tier pattern) |
