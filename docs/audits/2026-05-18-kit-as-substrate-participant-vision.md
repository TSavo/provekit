...
| D5 | Replace `render_migrated_source(target_surface)` with `dispatch_realize` invocation for each callsite. Depends on D5a verdict. | The 280+ lines of hardcoded fixture output (THE central reinvention) are deleted. The realize plugins emit the target source. | Migrate workflow becomes thin — composes primitives. | #1229 |
