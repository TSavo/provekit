...
|---|---|---|---|---|
| D6-A1 | Architect-call: BindLiftEntry SQL-content carrying convention (G1 Q1). Decide between term_shape encoding, new `concept_arguments` field, or witness-shaped carrying. Pin the convention in a ruling doc. | `kit_dispatch.rs:411-436` (BindLiftEntry shape). `walk_rpc.rs:392-413` (Rust emission shape). Ruling doc lives at `docs/plans/2026-05-19-bind-lift-concept-arguments-ruling.md`. | None | YES |
| D6-A2 | Architect-call: target output-path convention as binding-kit declaration (G5). Where in the kit declaration shape does `output_file` belong; does the language kit or the library kit own it. Pin in a ruling doc. | `cmd_bind_migrate.rs:287-292`, `2089`, `1339`. Ruling doc `docs/plans/2026-05-19-target-output-path-convention-ruling.md`. | None | YES |
| D6-A3 | Architect-call: structural shell handling for target-binding-agnostic migrate (G3). Decide between (a) per-target-binding scaffolding declarations, (b) source-to-source transformation as a substrate primitive, (c) rescoping the migrate to emit only callsite-level changes and assembling whole files in a separate workflow. Pin in a ruling doc. | `cmd_bind_migrate.rs::compose_migrated_source` and the helper functions it calls. Ruling doc `docs/plans/2026-05-19-migrate-shell-handling-ruling.md`. | None | YES |
