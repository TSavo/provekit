...
[source annotations]
   ...
   [kit lifter]           -- per-language; extracts contracts + call edges from source
        |
        v  (NDJSON RPC: sugar-lsp-plugin/1)
   [linker daemon]        -- sugar-linkerd; unions all kits; derives bridges
        |
        v
   [LSP server]           -- sugar-lsp; converts linker-error mementos to LSP diagnostics
        |
        v
   [editor]               -- red squiggle at the call site
```

**Kit lifter.** A binary that speaks the `sugar-lsp-plugin/1` NDJSON protocol over stdio. It receives `initialize` / `parse` / `shutdown` JSON-RPC calls and responds. The `parse` method receives `{uri, text}` for a source file and returns `{annotations: [...]}` plus optionally `{callEdges: [...]}`. The lifter does one thing: translate the host language's contract syntax into Sugar IR.

**Linker daemon (`sugar-linkerd`).** The daemon holds the union of all kits' contract and call-edge streams in memory. When a kit lifter (via the LSP server) sends a `parseFile` RPC, the daemon lifts the file, re-derives affected bridges, and returns `{diagnostics: [...]}`. The daemon owns cross-language linkage. It is the only component that sees all kits at once.

**LSP server (`sugar-lsp`).** A thin adapter: receives `textDocument/didOpen` and `textDocument/didChange` events from the editor, forwards them to the daemon via `parseFile` RPC when `--daemon-socket` is active, and calls `publishDiagnostics` with the results.
...
This builds the CLI (`sugar`), LSP server (`sugar-lsp`), linker daemon (`sugar-linkerd`), lifter (`sugar-lift`), and all supporting crates in one pass.

## Add a new kit lifter

A kit lifter is a binary that speaks `sugar-lsp-plugin/1`. The daemon dispatches to it by file extension.
...
```json
{"jsonrpc": "2.0", "id": 1, "result": {"name": "sugar-lsp-mylang", "version": "0.1.0"}}
```
...
The daemon's `lift_source` dispatch is in `implementations/rust/sugar-linkerd/src/methods.rs`. To add your kit:
...
Rust CLI SPAWNS one as a subprocess per `.sugar/lift/<surface>/manifest.toml`
...
- `initialize`
- `sugar.plugin.kit_declaration`, which advertises the kit's
  `protocol_version` (today `sugar-lsp-shared/1`), `kit_id`, and the list of
  methods it implements.

(`implementations/python/sugar-lift-py-pytest-witness/src/sugar_pytest_witness/lift_lsp.py:30,166`.)
...
- The sugar kit (`bind_rpc.py`) adds `sugar.plugin.recognize` and
  `sugar.plugin.materialize`.
  (`implementations/python/sugar-lift-python-source/src/sugar_lift_python_source/bind_rpc.py:84,90`.)
- Realize kits add `sugar.plugin.invoke`, `assemble`, `emit_module`,
  `body_template_entries`, `resolve_dependency_proofs`, and `check`.
  (`implementations/python/sugar-realize-python-requests/src/sugar_realize_python_requests/rpc.py`.)
- The witness kit adds `sugar.plugin.resolve_witness`, covered next.
...
```toml
resolve_witness_command = ["...", "python3", "-m", "sugar_pytest_witness.lift_lsp"]
resolve_witness_method = "sugar.plugin.resolve_witness"
```

(`examples/pytest-witness-dummy/.sugar/lift/python-pytest-witness/manifest.toml`.)
...
(`implementations/python/sugar-lift-py-pytest-witness/src/sugar_pytest_witness/lift_lsp.py:106-153`.)
...
`sugar.plugin.resolve_witness`, declares it in the manifest, and writes a
...
`implementations/python/sugar-lift-py-tests/src/sugar_lift_py_tests/editor_lsp.py`,
exposed as the `sugar-editor-lsp-python` console script. Unlike the batch
lift plugin (`lsp.py`, the sugar-lift/1 NDJSON protocol the CLI spawns per
...
It is a thin client over `sugar prove`. On open and save it runs `sugar
...
editor server (the rust `sugar-lsp` / `sugar-linkerd` binaries) is still
...
the `sugar_lsp` forward-propagator demo and the rust linker tests.
...
A lift adapter is a component of an existing kit lifter that recognizes a specific annotation library. For example, the Rust kit has `sugar-lift-proptest` (walks `proptest!` blocks) and `sugar-lift-contracts` (walks `#[requires]/[ensures]` macros). Adding a new adapter means adding a new pattern recognizer to an existing kit's lifter.

### Anatomy of a Rust lift adapter

```
implementations/rust/sugar-lift/src/
  adapters/
...
- **Go:** `implementations/go/sugar-lift-go-tests/`: uses Go AST via `go/ast` and `go/parser`.
- **Python:** `implementations/python/sugar-lift-py-tests/`: uses `ast` module from stdlib.
- **C#:** `implementations/csharp/Sugar.Lift.DataAnnotations/`: uses Roslyn `SyntaxTree` API.
- **Ruby:** `implementations/ruby/lib/sugar/lift/`: uses the `parser` gem.
...
implementations/
  rust/               -- canonical reference implementation
    sugar-cli/     -- CLI binary (sugar)
    sugar-lsp/     -- LSP server (sugar-lsp)
    sugar-linkerd/ -- linker daemon (sugar-linkerd)
    sugar-lift/    -- source lifter library + adapters
    sugar-linker/  -- pure linker algebra (no I/O)
    sugar-canonicalizer/ -- IR canonicalization (JCS + BLAKE3-512)
    ...
  go/
    sugar-ir-symbolic/   -- Go IR library
    sugar-self-contracts/ -- Go self-contracts mint
    sugar-lift-go-tests/ -- Go lifter (binary: sugar-lsp-go)
...
2. `protocol-verify`: runs `sugar verify-protocol --signed`, which checks that the local binary's declared catalog CID is signed by the foundation key.
3. `all-mint`: runs the Linux-profile per-kit mint commands. Each mints the kit's self-contracts bundle, computes the `contractSetCid`, and verifies it against the signed attestation envelope in `.sugar/self-contracts-attestations/<lang>.json`. Fails if the contractSetCid has drifted from the signed value.
...
- **A new kit lifter binary**: implement the `sugar-lsp-plugin/1` protocol, add a daemon dispatch arm, add conformance tests, open a PR.
