...
Configure your lift plugins in `.provekit/config.toml` plus a manifest under
`.provekit/lift/<surface>/manifest.toml`, then mint:

```sh
provekit mint --project . --out .
```
...
write_witness_package([w], ".provekit/witnesses")
```
...
(`implementations/python/provekit-lift-py-pytest-witness/src/provekit_pytest_witness/witness.py:156-164`.)
