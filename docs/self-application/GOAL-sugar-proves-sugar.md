# GOAL: sugar proves sugar, for real (the durable north star)
...
sugar is a verifier PRODUCT a real vendor deploys against their existing
code: a TypeScript shop, a Rust shop, a Java shop, a Go shop points sugar at
...
demo is that it proves ITSELF (snake-eats-tail on `sugar-cli` and
`libsugar`) AND the federation surface holds across the four kits that ship
...
rest of the language's ordinary vocabulary. Vendors do not learn sugar's
contract language first; sugar's lifters learn the vendor's ecosystem.
...
2. **Substantive.** On sugar's own code, K (provably panic-safe sites) covers
...
3. **Observable and self-checking.** The target verdict is one command
   (`sugar self-check`), pinned by a golden in CI, with `doctor` validating
   kit wiring up front and `sugar release-gate` producing the v1 evidence
...
6. **Vendor-deployable.** A vendor with an existing TypeScript / Rust / Java /
   Go codebase and existing unit tests points sugar at their project,
...
`sugar-cli` and `libsugar` by #1787. Criterion 5 (cross-language)
...
- Golden snapshot in CI: shipped (`sugar-cli/tests/self_check_golden.rs`
  under conformance gate; readable diff on regression).
- `sugar doctor` HARD-fails (exit 2) on the manifest method/phase footgun
...
- No-silent-failure floor PRs (recent arc):
  ...
  - **#1755** (closes #1754) mid-run `.sugar/imports/` mutation guard
  ...
  - **#1756** (#1749 walk envelope surface) threads `panic_loci` through the
    sugar-walk single-contract envelope mint path and keys `EnvelopeCache`
    by `contract_cid` plus panic-loci fingerprint, so header provenance cannot
    silently stale or disappear.
  - **#1758** (closes #1749) threads `panic_loci` through the sugar-lift
...
- **Production K measurement on `sugar-cli`** (convergent harness, K=3
  stable, 2026-06-01):
...
- **D-lib through D-fn slice queue complete through #1771.**
  ...
  - **PR-B (#1760, MERGED 2026-06-01):** libsugar rust-implications
    consumer enablement.
    `.sugar/lift/rust-implications/manifest.toml` + config.toml entry.
    Wires libsugar self-check to enumerate its own callsite obligations.
  ...
  - **PR-C (#1762, MERGED 2026-06-01):** per-type infallibility totality for
    the 4 confirmed libsugar D-lib sites. Per-crate
    ...
    metadata for external types). Result on libsugar self-check:
...
- **D-lib `&Value` for sugar-cli (PR-D, #1765, MERGED 2026-06-01).**
  ...
  Result on sugar-cli self-check: `panicSafe=6` (+2 kit_dispatch + 4
  libsugar imported via cross-target discharge), floor invariants intact.
  **Cumulative production K so far: 6 sites discharged via sound reasoning
  on real production code.**
- **C `json!` construction tracking for sugar-cli (#1767, MERGED
  ...
  sites. Result on sugar-cli self-check after syncing `sugar-realize-rust`:
...
- **B guarded panic partial propagation (#1769, MERGED 2026-06-01).** Rust kit
  ...
  libsugar self-check: `panicSafe=10`, `falsePass=0`,
  `silentlyDropped=0`, `droppedSites=[]`, `panicCensus=36`. Result on
  sugar-cli self-check: `panicSafe=19`, `falsePass=0`,
  `silentlyDropped=0`, `droppedSites=[]`, `panicCensus=53`; the +6 over
  post-#1767 is 5 B prelude/std-shim sites plus imported `libsugar`
...
- **D-fn cross-function postconditions (#1771, verified
  ...
  `.sugar/contracts/function_postconditions.toml` entries for
  ...
  Result on libsugar self-check: `panicSafe=12`, `falsePass=0`,
  `silentlyDropped=0`, `droppedSites=[]`, `panicCensus=35`,
  `oracle={requested:true, engaged:true, attempted:2740, resolved:2504}`.
  Result on sugar-cli self-check: `panicSafe=21`, `falsePass=0`,
...
- **Residue declaration (#1775, MERGED 2026-06-01).** Rust kit reads
  target-scoped `.sugar/residue.toml` entries and emits
  ...
  proven-site annotations. Result on libsugar self-check:
  ...
  on sugar-cli self-check: `panicSafe=21`, `falsePass=0`,
...
- **Phase 5 reproducible K re-baseline (#1896, MERGED 2026-06-02).** The
  ...
  sugar-cli-local Rust-kit `infallible_serialize.toml` entry for
  ...
  tier-to-close bucket to proven. Earlier sugar-cli K=21 / panicCensus=53
...
- **Readiness-signal closure (2026-06-03).** The convergence harness is
  ...
  with `/run/user/1000/sugar`, `/tmp/sugar`, and the rust-analyzer cache
  cleared, `serverStatus quiescent=true` arrived in 1s for shim-std, 4s for
  libsugar, and 4s for sugar-cli. The run reported
  `panicSafe=20`, `falsePass=0`, `silentlyDropped=0`, `droppedSites=[]`,
  `panicCensus=54`, `bridges.emitted=2832`, and
  `oracle={attempted:4173, resolved:4079}`. The dep-libsugar `.proof`
  ...
  remaining libsugar lifter rows; #1899 tracks replacing the self-check
...
- **Slice 31 catalog K multiplication (#1902, 2026-06-03).** The Rust kit
  ...
  The libsugar catalogs added audited serde/function entries for
  `BridgeHeaderV14`, `Witness`, `Term`, `Verb`, `IrFormula`,
  `serializable_jcs(&canonical)`, `json_jcs(&value)`, and `json_cid(&value)`.
  Cold self-checks landed the predicted close-list exactly: libsugar moves
  to `panicSafe=22` with `falsePass=0`, `silentlyDropped=0`, `droppedSites=[]`;
  sugar-cli moves to `panicSafe=32` with `falsePass=0`,
  `silentlyDropped=0`, `droppedSites=[]`. The predicted sugar-cli cascade was
  ...
  remaining libsugar gap census, and the independent sugar-cli rows move
...
- **Slice 32 guarded pure-free routing (#1905, 2026-06-03).** The Rust kit
  ...
  libsugar to `panicSafe=24` and sugar-cli to `panicSafe=34`, with
...
- **Doctor + release-gate arc:** #1779 refactors doctor around a reusable
  ...
  #1787 adds `sugar release-gate`.
- **Rust v1 executable claim:** `sugar release-gate --json` runs
  `doctor --mode releaseGate` and `self-check --json` on `sugar-cli` and
  `libsugar`, aggregates the receipt, and exits 0 only when the floor and
  target evidence are green. The v1 git tag itself is a separate release-policy
  decision; the evidence command now exists.

## Current reproducible census (sugar-cli, Rust self-application)
...
the product baseline. The current `sugar-cli` target reports
...
closed 10 imported libsugar-backed rows in the sugar-cli cascade,
...
libsugar source-transform guarded-panic rows via stable pure-free guard
...
|---|---|---|
| proven K | 34 panic-safe | Current reproducible K after readiness-signal gating replaces convergence, slice 31's catalog-backed libsugar cascade lands, and slice 32's stable pure-free guard facts close the two imported source-transform rows. D-lib, C `json!`, B guarded-panic, D-fn, audited catalog-backed serde/function entries, 2 solver-side `kit_dispatch` JSON-RPC request serialization closes, and 2 `cf_guarded` source-transform closes remain the closing mechanisms, measured against the cold bcargo + battleaxe RA baseline. |
| residue | 8 honest residue | Mutex `.lock().expect()` rows. Honest residue; "lock is total" would be unsound. |
| closeable tier-to-close | 0 named D-lib rows | #1896 closed the `RealizeRequest` serialization row with sugar-cli-owned per-type D-lib manifest coverage, mirroring libsugar's `infallible_serialize.toml` pattern. |
| independent sugar-cli unproven | 6 | `cmd_emit`, `cmd_mint`, `cmd_protocol` x3, and `doctor_oracle`. Tracked separately from #1898 in #1901. |
| imported libsugar parked rows | 6 | Remaining libsugar rows propagated through dependency import: the `compose.rs:1157` NoBridge physical site, the `platform_semantics.rs:42` external-runtime site, and 4 catalog-excluded solver rows. |
...
## Current census (libsugar, Rust v1 release gate)

PR-B (#1760) enabled the rust-implications consumer surface for libsugar and
...
Expected PR-C K delta on libsugar's self-check: **+4** confirmed D-lib
sites. The B and D-fn rows are later tiers, not PR-C.

Post-#1769 current libsugar score: `panicCensus=36`, `panicSafe=10`,
...
Post-#1771 main libsugar score: `panicCensus=35`, `panicSafe=12`,
...
Post-#1775 main libsugar score: `panicCensus=35`, `panicSafe=12`,
...
through stable pure-free `cf_guarded` facts. The remaining libsugar rows are
...
`sugar self-check` over sugar's own crates emits, deterministically:
...
- **D-lib per-type for libsugar** (in progress; three-PR split):
  ...
  - **PR-B (MERGED, #1760):** libsugar
    `.sugar/lift/rust-implications/manifest.toml` + `config.toml` entry.
    Enables libsugar self-check to enumerate its own callsite obligations.
  ...
  - **PR-C (MERGED, #1762):** per-type infallibility for 4 audited
    libsugar types (`RealizedSource`, `Sort`, `Dialect`, `Term`). +4 K
    delta on libsugar's self-check. First real production K via sound
    reasoning.
- **D-lib `&Value` for sugar-cli (MERGED, #1765):** 2 kit_dispatch sites
  discharge via &Value totality. Bundles 4 structural fixes (bundle
  provenance, panicLoci enumeration, dep proof RPC flow,
  bodyDischargeEligible metadata persistence). +2 K delta on sugar-cli.
- **C `json!` construction tracking (MERGED, #1767):** closes the 7
  ...
  +7 K delta on sugar-cli; cumulative production K=13.
- **B intra-fn `assert!` / iterator-length propagation (MERGED, #1769):**
  closes 5 prelude/std-shim sites plus the imported libsugar `wp.rs:295`
  ...
  delta on sugar-cli; cumulative production K=19.
- **D-fn cross-function postconditions (MERGED, #1771):** closes the 2
  ...
  +2 K delta on the earlier sugar-cli measurement, reaching historical
...
- **Residue declaration (MERGED, #1775):** target-scoped Rust-kit
  `.sugar/residue.toml` entries annotate panic census rows without
  changing discharge. 8 sugar-cli Mutex poisoning sites become
  ...
  sugar-cli-local infallible serialization metadata; the libsugar
...
- Rust v1 is executable: `sugar release-gate --json` validates
  `sugar-cli` and `libsugar` through doctor release-gate mode and
...
capabilities through a shared `sugar.plugin.kit_declaration` RPC, and route
...
2. **Ecosystem-native lifter coverage.** Lifters mine contracts from whatever
   ...
   codebase; they are not centralized in sugar-specific files.
...
reproducible Phase 5 baseline is K=34 on sugar-cli after slice 32 and K=24
on libsugar. Earlier sugar-cli K=21 references came from a different
...
1. **Dogfood depth first.** Drive Rust self-application K into vendor-
   ...
   `sugar-cli` and `libsugar`) via shim catalog expansion and discharge
   tier work. This
...
2. **Third-party Rust OSS project parity.** Once dogfood "feels eaten" by
   ...
   pitch artifact: "your existing tests pass; sugar additionally proved
...
numeric trigger. "Dogfood is gold standard" means sugar's own scoreboard
...
- Cross-crate audited shims for top ecosystem crates (serde, tokio, sqlx,
  ...
  catalog.
- Target: K from 32 to 100+ on `sugar-cli`, K from 22 to 50+ on
  `libsugar`, with ZERO source modifications.
...
- Annotations as proof prerequisites are out of scope entirely. A vendor
  should get nontrivial K from running sugar against an unmodified
...
- **Recent PRs (this arc)**:
  ...
  - #1756 (#1749 walk envelope) panic_loci threading + EnvelopeCache
    fingerprint key.
  - #1758 (#1749 lift direct) panic_loci threading through sugar-lift
    direct mint.
  ...
  - #1760 (D-lib PR-B) libsugar rust-implications consumer enablement +
    warm-oracle baseline scoreboard (`panicCensus=15`, confirmed D-lib=4).
  - #1762 (D-lib PR-C) per-type infallibility for 4 libsugar types.
    First real production K delta: +4 on libsugar's self-check.
  - #1765 (D-lib PR-D) `&Value` for sugar-cli + 4 structural fixes
    (verifier bundle provenance, panicLoci enumeration,
    `resolve_dependency_proofs` RPC flow, body-discharge metadata persistence).
    +2 K delta on sugar-cli. Cumulative production K: 6.
  ...
  - #1769 (B slice) Rust-kit guarded panic propagation for `assert!` facts and
    ...
    verifier no-scoped-bridge guard. +6 K delta on sugar-cli. Cumulative
    production K: 19.
  - #1771 (D-fn slice) manifest-backed cross-function postconditions for
    ...
    sugar-cli. Cumulative production K: 21.
  - #1775 (residue declaration) Rust-kit target-scoped residue manifest and
    `self-check` panic census annotation join. Names 8 sugar-cli Mutex
    residues, 1 sugar-cli D-lib tier-to-close row, and 1 libsugar
  ...
  - #1787 (doctor PR 7) `sugar release-gate` command and v1 evidence
    receipt.
  - #1902 (Slice 31 catalog K multiplication): Rust-kit manifest-backed
    lifter support plus libsugar serde/function catalog entries. Post-slice-31
    reproducible baselines: libsugar K=22 and sugar-cli K=32, with
    `falsePass=0`, `silentlyDropped=0`, and `droppedSites=[]` on both crates.
  - #1905 (Slice 32 guarded pure-free routing): stable pure-free
    `is_some()` guards now route the matching `unwrap()` obligations through
    `cf_guarded` facts. Current reproducible baselines: libsugar K=24 and
    sugar-cli K=34, with `falsePass=0`, `silentlyDropped=0`, and
...
- **Open follow-ups**:
  ...
  - #1786 update stale `--release-gate` doctor spelling to
    `--mode releaseGate`.
- **Key files**: `sugar-verifier/src/{runner.rs, enumerate_callsites.rs,
  body_discharge.rs, handshake.rs, load_all_proofs.rs}`,
  `sugar-walk/src/{lift.rs, bin/walk_rpc.rs, envelope.rs}`,
  `sugar-cli/src/{cmd_self_check.rs, cmd_mint.rs}`,
  `sugar-ir-compiler-smt-lib/src/generated.rs`,
  rust-std shim `examples/sugar-shim-rust-std`.
