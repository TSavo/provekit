# sugar lift v0: synthesize `.proof` from existing TS code
...
`src/proveLift/`. The CLI surface is `sugar lift <file>.ts`. The
...
1. End-to-end on parseInt: starting from a raw `parseInt.ts` (no
   `must()`, no `forall()`), `sugar lift parseInt.ts` produces a
   `.proof` file.
...
`sugar lift` is a new top-level subcommand in `cli.ts`, peer to
...
| `signing-key-missing` | no `SUGAR_KEY` env var or `--key` flag | 6 |
...
- **Auto-installing the proof in `node_modules`.** The user invokes
  `sugar lift`, gets a file, decides what to do with it.
