...
**Goal:** Build the first executable Bug Zoo slice: a Rust CLI `sugar zoo` runner plus one Java null-boundary specimen whose Sugar-native and ecosystem-native surfaces lift to the same boundary ProofIR and expose the missing `maybe_null(name) => non_null(name)` edge.
...
- `bug-zoo/species/BZ-SHAPE-005-java-null-boundary-equivalence/exposed/sugar-native/harness/src/main/java/com/sugar/contract/Requires.java` - tiny local Sugar-native annotation stub.
- `bug-zoo/species/BZ-SHAPE-005-java-null-boundary-equivalence/exposed/sugar-native/harness/src/main/java/zoo/UserDirectory.java` - Sugar-native Java contract surface.
...
- `implementations/java/sugar-lift-java-sugar-native/pom.xml` - Java kit module for Sugar-native annotations.
- `implementations/java/sugar-lift-java-sugar-native/src/main/java/com/sugar/lift/sugarnative/SugarNativeExtractor.java` - lifts `@Requires`, `@Ensures`, `@Invariant`.
- `implementations/java/sugar-lift-java-sugar-native/src/main/resources/META-INF/services/com.sugar.lift.Extractor` - service loader registration.
- `implementations/rust/sugar-cli/src/cmd_zoo.rs` - Rust CLI runner and JSON-RPC lifter client.
- `implementations/rust/sugar-cli/tests/zoo_smoke.rs` - CLI integration smoke test using the real specimen.

Modify:

- `implementations/java/pom.xml` - add `sugar-lift-java-sugar-native` module.
- `implementations/java/sugar-lift-java-integration-tests/pom.xml` - depend on the Sugar-native extractor.
- `implementations/java/sugar-lift-java-integration-tests/src/test/java/com/sugar/lift/CrossDomainContractEquivalenceTest.java` - add Sugar-native surface to existing equivalence tests.
- `implementations/rust/sugar-cli/Cargo.toml` - add `serde_yaml = "0.9"`.
- `implementations/rust/sugar-cli/src/main.rs` - add `Zoo` subcommand.
...
**Files:**
- Create: `implementations/java/sugar-lift-java-sugar-native/pom.xml`
- Create: `implementations/java/sugar-lift-java-sugar-native/src/main/java/com/sugar/lift/sugarnative/SugarNativeExtractor.java`
- Create: `implementations/java/sugar-lift-java-sugar-native/src/main/resources/META-INF/services/com.sugar.lift.Extractor`
- Modify: `implementations/java/pom.xml`
- Modify: `implementations/java/sugar-lift-java-integration-tests/pom.xml`
- Modify: `implementations/java/sugar-lift-java-integration-tests/src/test/java/com/sugar/lift/CrossDomainContractEquivalenceTest.java`
...
```java
import com.sugar.lift.sugarnative.SugarNativeExtractor;
```
...
```java
String sugarSource = """
    import com.sugar.contract.Requires;
...
```java
String sugarIr = lift(new SugarNativeExtractor(), sugarSource);
assertEquals(beanIr, sugarIr, "Sugar-native and Bean Validation IR must be identical");
```
...
```bash
mvn -f implementations/java/pom.xml -pl sugar-lift-java-integration-tests -am test -Dtest=CrossDomainContractEquivalenceTest
```

Expected: compilation fails because `com.sugar.lift.sugarnative.SugarNativeExtractor` does not exist.
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <parent>
        ...
        <artifactId>sugar-lift-java-parent</artifactId>
        <version>0.1.0</version>
    </parent>
    <artifactId>sugar-lift-java-sugar-native</artifactId>
    <packaging>jar</packaging>
    <dependencies>
        <dependency>
            <groupId>com.sugar</groupId>
            <artifactId>sugar-lift-java-core</artifactId>
            <version>0.1.0</version>
        </dependency>
    </dependencies>
</project>
...
Add module in `implementations/java/pom.xml` immediately after `sugar-lift-java-core`:

```xml
<module>sugar-lift-java-sugar-native</module>
```

Add dependency in `sugar-lift-java-integration-tests/pom.xml`:
...
Create `SugarNativeExtractor.java`:

```java
package com.sugar.lift.sugarnative;

import java.util.*;
import com.github.javaparser.ast.*;
import com.github.javaparser.ast.body.*;
import com.github.javaparser.ast.expr.*;
import com.sugar.lift.*;

public class SugarNativeExtractor implements Extractor {
    public String name() { return "sugar-native"; }
    ...
    private String toIr(String expr) {
        ...
        return "{\"kind\":\"atomic\",\"name\":\"sugar_predicate\",\"args\":[{\"kind\":\"const\",\"value\":\"" + esc(e) + "\",\"sort\":{\"kind\":\"primitive\",\"name\":\"String\"}}]}";
    }
...
Create service file `META-INF/services/com.sugar.lift.Extractor`:

```text
com.sugar.lift.sugarnative.SugarNativeExtractor
```
...
```bash
git add implementations/java/pom.xml implementations/java/sugar-lift-java-sugar-native implementations/java/sugar-lift-java-integration-tests/pom.xml implementations/java/sugar-lift-java-integration-tests/src/test/java/com/sugar/lift/CrossDomainContractEquivalenceTest.java
git commit -m "feat(java): lift sugar-native contracts"
```
...
Create `exposed/sugar-native/harness/src/main/java/com/sugar/contract/Requires.java`:

```java
package com.sugar.contract;
...
Create `exposed/sugar-native/harness/src/main/java/zoo/UserDirectory.java`:
...
Create `exposed/sugar-native/kit-rpc/run-java-lifter.sh` and `exposed/spring-web/kit-rpc/run-java-lifter.sh` with identical content:
...
java -cp "$repo_root/implementations/java/sugar-lift-java-core/target/sugar-lsp-java.jar:$repo_root/implementations/java/sugar-lift-java-sugar-native/target/sugar-lift-java-sugar-native-0.1.0.jar:$repo_root/implementations/java/sugar-lift-java-spring-web/target/sugar-lift-java-spring-web-0.1.0.jar" \
  com.sugar.lift.Main --rpc
```

Mark both executable:

```bash
chmod +x bug-zoo/species/BZ-SHAPE-005-java-null-boundary-equivalence/exposed/sugar-native/kit-rpc/run-java-lifter.sh
...
Create both `exposed/sugar-native/expected.proofir.json` and `exposed/spring-web/expected.proofir.json`:
...
- `sugar-native`: explicit Sugar-style `@Requires("name != null")`
...
**Files:**
- Modify: `implementations/rust/sugar-cli/Cargo.toml`
- Create: `implementations/rust/sugar-cli/src/cmd_zoo.rs`

- [ ] **Step 1: Add dependency**

Add to `implementations/rust/sugar-cli/Cargo.toml`:
...
"#;
        ...
        assert_eq!(manifest.equivalence.required[0], ["sugar-native", "spring-web"]);
    }
}
```
...
cargo test -p sugar-cli cmd_zoo::tests::parses_manifest_with_exposures_and_equivalence
```
...
```bash
git add implementations/rust/sugar-cli/Cargo.toml implementations/rust/sugar-cli/src/cmd_zoo.rs
...
**Files:**
- Modify: `implementations/rust/sugar-cli/src/cmd_zoo.rs`
...
fn validation_rejects_missing_lossiness() {
    let manifest = SpecimenManifest {
        ...
        expectations: Expectations { host_compiler: "pass".into(), ordinary_tests: "pass".into(), sugar_verify: "fail".into() },
...
```bash
cd implementations/rust
cargo test -p sugar-cli cmd_zoo::tests
```
...
```bash
git add implementations/rust/sugar-cli/src/cmd_zoo.rs
...
**Files:**
- Modify: `implementations/rust/sugar-cli/src/cmd_zoo.rs`
- Modify: `implementations/rust/sugar-cli/src/main.rs`
...
fn invoke_lift_rpc(specimen_dir: &Path, exposure: &Exposure) -> Result<Value, String> {
    ...
    let init_req = json!({
        ...
        "method": "initialize",
        "params": {
            "client": {"name": "sugar-zoo", "version": env!("CARGO_PKG_VERSION")},
...
fn proof_ir_cid(value: &Value) -> Result<String, String> {
    ...
    let bytes = sugar_canonicalizer::encode_jcs(&canonical).into_bytes();
    Ok(sugar_canonicalizer::blake3_512_of(&bytes))
}

fn json_to_cvalue(j: &Value) -> std::sync::Arc<sugar_canonicalizer::Value> {
...
```bash
git add implementations/rust/sugar-cli/src/main.rs implementations/rust/sugar-cli/src/cmd_zoo.rs implementations/rust/sugar-cli/Cargo.toml
...
**Files:**
- Create: `implementations/rust/sugar-cli/tests/zoo_smoke.rs`
...
fn java_null_boundary_specimen_checks() {
    ...
    assert!(json["reports"][0]["proofIrCids"]["sugar-native"].as_str().unwrap().starts_with("blake3-512:"));
    assert_eq!(
        json["reports"][0]["proofIrCids"]["sugar-native"],
...
```bash
mvn -f implementations/java/pom.xml -pl sugar-lift-java-core,sugar-lift-java-sugar-native,sugar-lift-java-spring-web -am package -DskipTests
```
...
```bash
cd implementations/rust
cargo run -p sugar-cli -- zoo ../../bug-zoo/species/BZ-SHAPE-005-java-null-boundary-equivalence
```
...
zoo: equivalence sugar-native == spring-web PASS
...
```bash
cd implementations/rust
cargo test -p sugar-cli --test zoo_smoke
```
...
```bash
git add implementations/rust/sugar-cli/tests/zoo_smoke.rs bug-zoo/species/BZ-SHAPE-005-java-null-boundary-equivalence/exposed/*/kit-rpc/run-java-lifter.sh
...
```bash
mvn -f implementations/java/pom.xml -pl sugar-lift-java-core,sugar-lift-java-sugar-native,sugar-lift-java-spring-web,sugar-lift-java-integration-tests -am test
cd implementations/rust
cargo test -p sugar-cli cmd_zoo::tests
cargo test -p sugar-cli --test zoo_smoke
cargo run -p sugar-cli -- zoo ../../bug-zoo/species/BZ-SHAPE-005-java-null-boundary-equivalence --json
```
...
- `sugar zoo ... --json` returns `"ok": true`.
- The JSON has identical CIDs for `sugar-native` and `spring-web`.
...
- Rust CLI orchestration: Tasks 3-6 add `sugar zoo`.
