...
## Sugar is not a substitute for testing

Tests verify behavior on a finite point set: the inputs you wrote. Properties hold over an input domain. Sugar addresses the second; it does not replace the first.
...
Each of these can fail. None is unconditional. Sugar does not produce a proof that all five are correct simultaneously. It produces a portable artifact that, conditional on those assumptions, asserts a behavioral property.

If a regulator requires Coq, F\*, Lean, or Isabelle output, those tools remain the right choice. Sugar is not them.

## Sugar is not a substitute for human review
...
Sugar provides the substrate for these decisions. The decisions remain human.

## Sugar is not a runtime monitor
...
## Sugar does not turn empirical software into mathematical software
...
Sugar does not turn this empirical software into mathematical software. It turns one specific class of behavioral verification (the kind expressible in the IR's logical fragments) into a content-addressed substrate that composes across the dependency graph.
...
## Sugar does not solve the cold-start problem
...
Early adopters of Sugar face an empty lattice. Their first runs are mostly Tier 3, mostly slow, with low Tier 1 fraction. The lattice grows from each project that adopts; the cost amortizes; the discharge fraction approaches the theoretical asymptote.

Sugar does not magically populate the lattice. See [`cold-start.md`](cold-start.md) for the bootstrap discussion.

## Sugar does not have zero adoption cost
...
A claim that "Sugar replaces the solver" misreads the architecture. Sugar invokes the solver, captures the result, distributes the result, and ensures every future verifier reuses the result. The solver still runs.
...
## Sugar does not provide a correctness audit
...
Sugar does not detect honest mistakes; it does not detect adversarial misrepresentation. The signature attests to the *signer*; it does not attest to *correctness*.

What Sugar provides is non-repudiation (the signer cannot deny having signed the contract) and tamper-evidence (the contract cannot be altered after signing without breaking the signature). These are useful, but they are not a correctness audit.
...
Sugar makes (1) cheap and (2) verifiable but does not perform either for you.

## Sugar is not Bitcoin
...
Specifically: Sugar does not have a token, does not have miners, does not have a consensus mechanism, does not have block reorganizations. The "registry" is a passive set of mementos that any peer can store; there is no party whose downtime breaks anything; there is no adversarial validation game.
...
- [../security/threat-model.md](../security/threat-model.md) (when written): what Sugar catches and what it doesn't.
