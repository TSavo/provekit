...
- `sugar-lift-c-kernel-doc`: lifts `/** kernel-doc */` comments. Empirical: rxkad.c has 1, kernel mostly has none on internal functions.
- `sugar-lift-c-sparse`: lifts sparse annotations (`__must_hold`, `__acquires`, `__user`, `__rcu`). Empirical: rxkad.c has 0, sibling files have 0-10 each.
- `sugar-lift-c-assertions`: lifts `assert()` / `BUG_ON` / `WARN_ON` (per name). Not yet verified empirically against kernel C.
...
### `sugar-walk` (Rust): the model

`implementations/rust/sugar-walk/src/lift.rs` does real predicate derivation from Rust source via `syn`:
...
Pre and post end up as real `IrFormula` (the canonical ProofIR formula type) and feed `libsugar::compose::FunctionContractMemento`.
...
### Deliverable 1: `sugar-lift-c-collectors-defensive` (new C lifter)

New C lifter at `implementations/c/sugar-lift-c-collectors-defensive/`, mirroring the sibling C lifter pattern (kernel-doc, sparse, assertions). Emits synthesized contracts as `declarations[]` entries via the same JSON-RPC `lift` method.
...
- `src/patterns.c`: C-side pattern recognition (port of `sugar-walk::lift::lift_function_precondition` / `lift_function_postcondition` algorithms to libclang cursor types)
- `src/formula.c`: builds the JSON shape that deserializes to `sugar_ir_types::IrFormula` on the Rust consumer side
- `Makefile`: same shape as `sugar-lift-c-kernel-doc` Makefile (libclang-enabled with effects.c, fallback to stub)
- `tests/integration.sh`: smoke tests on fixture C files (BUG_ON, if-return-error, assert patterns)

**JSON-RPC surface:** `c-collectors-defensive`. Reads source files, emits `pk_c_lift_result`-shaped JSON with synthesized contracts in `declarations[]`. Plug-compatible with existing `sugar lift` / `sugar compose --rpc` consumers.

**Why C instead of Rust:** keeps the lifter in `implementations/c/` family (consistent build, test, RPC pattern). `clang_ast.c` already does the libclang AST traversal: synthesis adds a new pass over the existing visitor, no clang-sys binding overhead. `effects.c` already linked in (per #519). The pattern recognition algorithms transfer from `sugar-walk::lift` regardless of language; we don't get to reuse `lift.rs` source either way (it's tightly bound to `syn::Expr`/`syn::Stmt` types). The C path is ~800-1200 lines of new code; Rust would be ~1500+ (clang-sys bindings + visitor + algorithm port).

**Formula representation:** the contracts emitted in `declarations[]` use the JSON shape that the canonical `sugar_ir_types::IrFormula` deserializes from. The Rust consumer side (sugar compose, prove) sees them as ordinary IrFormula values and feeds them into `libsugar::compose::FunctionContractMemento`. No new schema, no FFI from C side.

### Deliverable 2: `sugar-lift-c-kunit` (new C lifter)
...
**Plug shape:** new C lifter binary, same Makefile pattern as `sugar-lift-c-kernel-doc`. Surface name: `c-kunit`. Consumes through `sugar lift / compose / prove` unchanged.

### Deliverable 3: Extension to `sugar-lift-c-assertions`
...
|---|---|---|---|---|
| (a.1) Trivial | `{pre=true, post=true, effects=<from libclang>}` per function | All functions in lifted files | Smallest. Just package effects.c output as contract. | `sugar-lift-c-collectors-defensive` skeleton |
| (a.2) Pattern-derived | Real preconditions from BUG_ON, if-return-error, assert, goto-error | All functions with defensive patterns (most kernel functions have at least one) | Medium. Port lift.rs algorithms C-side. | `sugar-lift-c-collectors-defensive` + extended assertions lifter |
| (a.3) Test-derived | Real semantic claims from KUnit assertion macros | All functions covered by KUnit tests | Medium. Mirror JUnitExtractor. | `sugar-lift-c-kunit` |
| (a.4) Type-derived | Constraints from `__user`, `__rcu`, `__must_hold(lock)`, `size_t`, `loff_t`, `gfp_t`, `__bitwise`, etc. | All functions touching kernel-typed parameters (essentially every kernel function) | Medium-hard. Per-attribute predicate library. | `sugar-lift-c-collectors-defensive` extension (folds into PR 2) |

## PR sequence with explicit gates

### PR 1: `sugar-lift-c-collectors-defensive` skeleton + (a.1) trivial synthesis

**Deliverable:**
- New C lifter `implementations/c/sugar-lift-c-collectors-defensive/` with src/main.c (RPC scaffold copied from kernel-doc lifter), src/walker.c (synthesis driver), Makefile (libclang + effects.c, mirror kernel-doc Makefile)
...
2. `sugar compose --rpc` consumes the declarations → expect composed contracts emitted
3. `sugar prove --formula <one_composed>` returns a verdict (will be trivial discharge since pre=post=true, but pipeline is end-to-end)

**Merge condition:** all three smoke steps green. Trivial discharge is OK at this tier.

### PR 2: (a.2) defensive-pattern derivation + (a.4) type-derived predicates in `sugar-lift-c-collectors-defensive`

**Deliverable:**
- Port `sugar-walk::lift` algorithms to `src/patterns.c`:
  ...
  - De Morgan + double-neg elim + comparison flips (mirror sugar-walk::lift::negate)
...
4. Hand-craft a violating caller for one of the above and verify `sugar prove` REFUTES the composed implication with a counterexample

**Merge condition:** at least one real refutation observed end-to-end across (a.2) and (a.4) combined.

### PR 3: `sugar-lift-c-kunit` mirror of JUnitExtractor

**Deliverable:**
- New C lifter at `implementations/c/sugar-lift-c-kunit/`
...
### PR 4: Extend `sugar-lift-c-assertions`
...
1. **Where does `sugar-walk-c` live?** Sibling crate `implementations/rust/sugar-walk-c/`, or extension module inside `sugar-walk` (`sugar-walk::c`)? Sibling probably cleaner: different parser, different binary, different test fixtures.
...
5. Compose via canonical libsugar compose → composed contracts for every chain
...
- Implementing `sugar-walk-c` (this is the design; implementation comes after Sir's review)
