# ProvekIt: `provekit migrate` Verb (catalog version bumps)
...
The behavior of `provekit migrate <new-version>`, a CLI verb that automates the mechanical steps required to cut a new minor catalog version (e.g. v1.3.0 to v1.4.0). The verb is a workflow runner, not a protocol participant. It produces no on-wire bytes; it edits a fixed list of files in lockstep, then exits.
...
```
provekit migrate <new-version> [--from <prev-version>] [--declared-at <iso8601>] [--force-dirty] [--dry-run]
```
...
5. **`.provekit/catalog-signatures/v<X>.<Y>.<Z>.json`** signed attestation.
...
6. **`implementations/rust/provekit-cli/assets/catalog-signature-v<X>.<Y>.<Z>.json`** embedded asset.
...
7. **`implementations/rust/provekit-cli/assets/protocol-catalog.json`** refreshed catalog.
...
8. **`implementations/rust/provekit-cli/src/protocol.rs`** version constants.
...
9. **`implementations/rust/provekit-cli/src/main.rs`** version docstring.
...
The verb stages every file write into a temporary shadow directory under `.provekit/migrate-staging/<new-version>/` and only flushes the staged tree onto the live working tree as the final operation, after step 13 verifies. Concretely:

1. Create `.provekit/migrate-staging/<new-version>/` (empty; if it exists from a prior failed run, the verb refuses unless `--force-dirty` is passed).
...
- **Not a continuous-integration entry point.** The migration produces a working tree intended for human review and a deliberate commit. CI MAY run `recompute-spec-cids` (default safe mode) and `provekit verify-protocol --signed` to confirm a freshly-pulled tree is internally consistent, but it MUST NOT invoke `provekit migrate` automatically.
...
5. After a successful run, `provekit verify-protocol --signed` against the live tree exits 0.
