...
export class PropertyTestChecker implements Checker {
  ...
  private runOne(contract: Contract, proven: ProvenProperty): CheckResult | null {
    ...
    let outcome: { kind: "returned"; value: any } | { kind: "threw"; error: string };
    try {
      const value = extracted.fn(...args);
      outcome = { kind: "returned", value };
    } catch (e: any) {
      outcome = { kind: "threw", error: String(e?.message || e).slice(0, 120) };
    }
    ...
    const outcomeSummary =
      outcome.kind === "returned"
        ? `returned ${this.formatValue(outcome.value)}`
        : `threw: ${outcome.error}`;
  ...
  private loadFunction(filePath: string, fnName: string): ExtractedFn | null {
    ...
    const paramNames = this.extractParamNames(filePath, fnName);
    if (!paramNames) {
    ...
    const result = { fn, paramNames };
    this.fnCache.set(cacheKey, result);
    return result;
  }

  private extractParamNames(filePath: string, fnName: string): string[] | null {
    try {
      ...
      const paramsNode = (target as Parser.SyntaxNode).childForFieldName("parameters");
