  ...
  execute(_input: void, options: PhaseOptions): PhaseResult<AxiomReport> {
    ...
    if (contracts.length === 0) {
      ...
      const report: AxiomReport = {
        ...
        errors: 0, ignored, consistency: "n/a", staleContracts: 0, results: [],
    ...
    for (const c of consistency) {
      if (c.verdict === "proven") this.detail(`✓ ${c.description}`);
      else if (c.verdict === "violation") this.detail(`✗ INCONSISTENCY: ${c.description}`);
      else this.detail(`⚠ ${c.description} -- ${c.error?.slice(0, 60)}`);
    }
    ...
    if (stale.length === 0) {
      ...
      for (const s of stale) {
        this.detail(`⚠ STALE: ${s.function}:${s.line}`);
      }
    }

    console.log();
    this.detail(`${contracts.length} contracts, ${results.length} checks`);
    this.detail(`${proven} proven | ${violations} violations | ${errors} errors${ignored > 0 ? ` | ${ignored} ignored` : ""}`);
