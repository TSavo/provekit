...
- the working tree IS the provekit repo itself (root `package.json` `name === "provekit"`)

If none of these hold, do not use the CLI; write code directly. ProvekIt is opt-in; do not impose it on projects that haven't adopted it.

**Announce at start when the skill is engaged:** "Using ProvekIt to make this change so it ships with its proof."
...
| User asks                                       | Use                                              |
|-------------------------------------------------|--------------------------------------------------|
| "Fix this bug"                                  | `provekit fix "<description>"`                   |
| "Add a feature" / "Implement X"                 | `provekit change "<description>"`                |
| "This code should never X"                      | `provekit must <file> "<must-not-X>"`            |
| "Verify this is correct"                        | `provekit attest`                                |
| "Find a case where this breaks"                 | `provekit refute <propertyHash>`                 |
| "Relax this invariant"                          | `provekit weaken <name> "<new-form>"`            |
| "Tighten this invariant"                        | `provekit strengthen <name> "<new-form>"`        |
| "Deprecate this invariant"                      | `provekit retire <name> "<reason>"`              |
| "Show me what's verified"                       | `provekit explain <propertyHash>`                |
| "What does my proof DAG depend on?"             | `provekit roots`                                 |
| "What did this codebase contribute?"            | `provekit leaves`                                |
| "Migrate to a new kit version"                  | `provekit migrate <oldHash> <newHash>`           |
| "Extract a reusable principle"                  | `provekit principalize <pattern>`                |

`provekit fix` and `provekit change` are sibling verbs over the same pipeline: `fix` for closing a bug, `change` for adding/modifying behavior. Pick the one whose framing matches the user's request; the framework treats both as content-addressed change proposals.
...
[provekit produces: patch (validator fix) + regression test + invariant; commits the bundle as one artifact]
...
When the user wants an invariant added without a code change, run `provekit must <file> "<intent>"`. The `must` workflow:
...
- **Editing files directly when `provekit fix` or `provekit change` would handle it.** The proof DAG only accumulates when changes flow through the workflow runner.
- **Hand-editing `.invariant.ts` files to add invariants.** Use `provekit must` so the lifter computes the canonical propertyHash.
- **Bypassing the commit gate with `--no-verify`.** The gate IS the framework's value; skipping it makes the commit unverifiable. If the gate is failing, fix the underlying issue or use `provekit refute` to surface the counterexample, then propose a real `provekit fix`.
- **Authoring invariants in the legacy `provekit/ir` builder API.** Use `provekit/ir/symbolic` with `must("<name>", ...)` instead.
...
| Situation                                                             | Action                                          |
|-----------------------------------------------------------------------|-------------------------------------------------|
| Project has `.provekit/` or `provekit` in deps                        | Skill applies; use the CLI verbs above          |
| Project is the provekit repo itself                                   | Skill applies                                   |
| Project has neither                                                   | Skill does not apply; write code directly       |
| User asks for a bug fix                                               | `provekit fix "<intent>"`                       |
| User asks for new behavior                                            | `provekit change "<intent>"`                    |
| User asks for an invariant only (no code change)                      | `provekit must <file> "<intent>"`               |
| User asks "is this correct?" / pre-commit verification                | `provekit attest`                               |
| Commit gate fails with an unclear failure                             | `provekit refute <propertyHash>` to get a counterexample |
| User wants to see what a propertyHash means                           | `provekit explain <propertyHash>`               |
| User wants the local↔external boundary of the proof DAG               | `provekit roots` (external) / `provekit leaves` (local) |

## Red flags

**Never:**
- Apply a patch from your own head when ProvekIt is installed and a `fix`/`change` verb would handle it.
