# libsugar Rust Surface Audit

## Summary

Audit scope was `implementations/rust/libsugar/src` plus the direct sibling crates named in `libsugar/Cargo.toml`: `sugar-canonicalizer`, `sugar-proof-envelope`, and `sugar-ir-types`. The CLI `sugar lift` in this branch dispatches configured plugins and does not expose a `--language rust` flag, so classification used the current direct Rust lifter surface in `sugar-walk` (`rust_function_term_json`, `type_decl`, and bind-lift behavior).
...
- `libsugar::core::primitives::address`
- `libsugar::core::traits::impl HashMapCatalog::insert`
- `libsugar::core::types::impl ArityShape::named`

### statement-macro (5 items)

- `libsugar::compose::impl AliasingMemento::to_jcs_value`
- `libsugar::core::types::impl Cid::from_hash_output`
- `sugar-canonicalizer::hash::tests::distinct_inputs_distinct_hashes`
- `sugar-canonicalizer::jcs::tests::empty_object_and_array`
- `sugar-proof-envelope::sign::tests::verify_rejects_malformed`

### nested-item (3 items)

- `sugar-ir-types::lib::migration_json_to_canonical`
- `sugar-ir-types::lib::proof_run_json_to_canonical`
- `sugar-ir-types::lib::serde_json_to_canonical_value`
...
- `libsugar::compose::impl std::fmt::Display for OpacityError`
- `libsugar::core::types::impl fmt::Display for Cid`
- `libsugar::desugar::impl fmt::Display for Refusal`
- `libsugar::desugar::impl std::error::Error for Refusal`
- `sugar-ir-types::lib::impl std::error::Error for CanonicalizationExtensionError`

### impl-associated-type-not-lowered (12 items)

- `libsugar::compose::impl TryFrom<&FunctionContractMemento> for sugar_ir_types :: DomainClaim::Error`
- `libsugar::core::types::impl TryFrom<&str> for Cid::Error`
- `libsugar::core::types::impl TryFrom<DomainClaim> for Refutation::Error`
- `libsugar::core::types::impl TryFrom<DomainClaim> for Truth::Error`
- `libsugar::core::types::impl TryFrom<String> for Cid::Error`
...
- `libsugar::core::types::impl Deserialize<'de> for Cid`
- `sugar-ir-types::lib::impl Deserialize<'de> for PolicyMemento`

### complex-generic (1 items)

- `libsugar::core::traits::Domain::discharge`

### impl-associated-const-not-lowered (1 items)

- `sugar-ir-types::lib::impl DomainClaim::KIND`

### type-sort-opaque (1 items)

- `libsugar::core::traits::DischargeMode`
...
- Direct dependency crates are included only because `libsugar` composes them through its manifest. Other workspace consumers of `libsugar`, such as `sugar-mint-amp`, are outside this D1 surface pass.
...
Each row is caused by a function-local `use sugar_canonicalizer::Value as CanonicalValue;` item. D5 treats function-local item statements as non-executable for term emission, so they no longer cause a `nested-item` refusal. Any remaining refusal for these functions belongs to later unsupported expression classes, not to `nested-item`.
