...
RAW="bootstrap/libprovekit-bind-inventory/raw"
...
RECEIPT="$RECEIPT_DIR/libprovekit-intra-vendor.json"
CLI="implementations/rust/target/release/provekit"
THRESHOLD=4

TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/provekit-witness-consensus.XXXXXX")"
...
echo "build provekit-cli"
(cd implementations/rust && cargo build --release -p provekit-cli)
...
echo "run mint"
set +e
PROVEKIT_BIND_OUTPUT="$RAW/" bash menagerie/concept-shapes/mint.sh > "$MINT_OUT" 2>&1
mint_rc=$?
set -e
tail -n 20 "$MINT_OUT"
...
receipt = {
    "kind": "libprovekit-intra-vendor-witness-consensus-receipt",
    "schemaVersion": "1",
...
{
  printf '%s\n' "bootstrap/scripts/run_libprovekit_intra_vendor_consensus.sh"
  printf '%s\n' "$RECEIPT"
